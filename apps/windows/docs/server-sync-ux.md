...
사용자 확인 기준으로 WPF 공통 SQLite에는 `SYNCED` 387건, `FAILED` 277건이 있었다. 서버 연결 스모크 테스트까지 누적된 2026-07-09 KST 현재 재조회 기준은 `SYNCED` 520건, `FAILED` 284건이다. 테스트 이력 보존 규칙에 따라 기존 큐와 SQLite 기록은 삭제하지 않는다.
...
- 서버 URL 미설정 0건: 2026-07-09 서버 연결 스모크 재시도에서 기존 서버 URL 미설정 실패는 서버 연결 상태로 처리되어 실패 그룹에 남아 있지 않다.
