...
1. 운영 HTTPS 서버 연결을 의도적으로 차단한 상태에서 문서를 등록한다. 개발 PC에 로컬 FastAPI를 띄우거나 HTTP 주소를 사용하지 않는다.
...
7. 운영 HTTPS 연결을 복구하고 같은 서버 계정으로 로그인한 뒤 `재시도`를 누른다.
...
WPF 스모크 테스트는 운영 HTTPS 연결 실패, 재시도 성공, `server_id_mappings` 생성, 중복 큐 방지를 검증한다. 수동 검증은 로컬 클라이언트 로그와 공통 SQLite 이력을 보존하되 운영 서버 DB·storage·로그 사본은 개발 PC에 남기지 않는다.
