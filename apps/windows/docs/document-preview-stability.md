...
| XLSX | `품질점검표-라인A-대용량.xlsx` | 5 MiB 이상 익명 엑셀 양식 | 최대 100행 미리보기와 공통 열람/종료/차단 로그 |
| 이미지 | `사진-설비점검-라인A-고해상도.bmp` | 5 MiB 이상 고해상도 익명 이미지 | 앱 종료 없이 표시 또는 한글 실패 안내와 공통 로그 |
...
- 대용량 TXT, 손상 PDF, 큰 XLSX, 고해상도 이미지, 미지원 CAD/HWP의 익명 예외 샘플 목록이 정의되어 있다.
