...
문서 보기 창에서 새 코멘트를 저장하면 문서 버전을 증가시키지 않고 `field_notes`에 원천 이력으로 저장한다. 선택한 사진/파일 첨부는 `field_note_attachments`와 로컬 파일 경로에 저장한다.
...
서버 URL과 Bearer token이 있으면 문서, FieldNote, FieldNote 첨부, 접근 로그 전송 후보를 `server_sync_queue`에 남기고 즉시 재시도한다. 실패해도 로컬 저장은 성공으로 유지한다.
