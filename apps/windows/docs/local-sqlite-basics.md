...
Windows WPF 앱은 문서, FieldComment, 첨부, 접근 로그, 보고서 같은 로컬 원천을 SQLite에 먼저 남기고 설정한 승인 HTTPS 주소로 동기화를 시도한다. 작업순서는 예외로 FastAPI snapshot을 권위 원천으로 직접 사용하며, 이 문서의 로컬 작업순서 테이블은 기존 기록·오프라인 읽기 캐시·초안으로만 보존한다.

테이블과 동기화 설명은 2026-08-09 현재 `FlowNoteLocalDatabase`와 연결 서비스 코드 기준이다.
...
로컬 SQLite 계정 row와 사용자 관리 화면은 기존 데이터·단위 테스트 호환을 위해 남아 있으며 현재 표준 로그인에서는 열지 않는다. 서버 로그인한 `admin`, `system-admin`은 서버 계정 화면과 FastAPI API로 계정 발급, 잠금·비활성화, 비밀번호 재설정, role 변경과 세션 폐기를 수행하며 로컬 계정 row를 덮어쓰지 않는다. 서버 PC의 `app.ops.server_accounts`는 초기·비상 운영 경로로만 유지한다.
