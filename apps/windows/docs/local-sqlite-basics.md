...
Windows WPF 앱은 서버 연결 여부와 관계없이 현장 문서 등록, 열람, FieldComment, 첨부, 작업순서, 이력 저장을 로컬 SQLite에 먼저 기록한다. 서버가 설정되어 있으면 로컬 저장 이후 동기화를 시도한다.
...
- `field_comments`: 현장 코멘트 원천 이력
- `field_comment_attachments`: FieldComment 사진/파일 첨부와 로컬 보존 경로
- `document_view_logs`: 문서 열람 시작/닫힘 감사 로그와 서버 로그 ID 후보
- `activity_history`: 폴더, 문서, FieldComment, 첨부, 동기화, 작업순서 변경 이력
- `tag_definitions`, `document_tags`: 문서 태그 사전과 문서-태그 연결
- `notifications`: 문서/FieldComment 관련 로컬 알림
...
- `server_sync_queue`: 문서, FieldComment, 첨부, 접근 로그 전송 후보와 실패/성공 상태
...
- `FieldCommentService`: FieldComment와 첨부 저장/조회
...
로컬 저장은 서버 전송보다 우선한다. 서버 URL이 없거나 전송에 실패해도 로컬 문서, FieldComment, 첨부, 접근 로그는 보존한다. 실패 사유는 `server_sync_queue.last_error`와 `activity_history`에 기록한다.
...
- `field_comments.server_comment_id`
- `field_comments.synced_at`
- `field_comment_attachments.server_attachment_id`
- `field_comment_attachments.synced_at`
