...
public static class DocumentPreviewPolicy
{
    ...
    [
        ...
        new(
            ...
            "PDF",
            DocumentPreviewKind.Pdf,
            "암호 PDF 또는 읽기 권한 실패와 같은 익명 PDF 실패 샘플",
            "앱을 종료하지 않고 PDF 읽기 실패 안내와 document.preview_failed 이력을 남긴다.",
            true),
        new(
            "TXT",
            "긴 경로/공백",
            "작업표준서 (혼합 공정)-긴 경로.txt",
            "Text",
            DocumentPreviewKind.Text,
            "한글, 공백, 괄호가 포함된 긴 로컬 경로의 익명 텍스트",
            "경로 해석 실패 없이 열람/종료/다운로드 차단 로그를 남긴다.",
            false),
        new(
            "XLSX",
            "큰 파일",
        ...
        new(
            ...
            "Image",
            DocumentPreviewKind.Image,
            "이미지 확장자이나 디코더가 읽을 수 없는 익명 파일",
            "앱을 종료하지 않고 이미지 손상 안내와 document.preview_failed 이력을 남긴다.",
            true),
        new(
            "CAD",
            "미지원",
