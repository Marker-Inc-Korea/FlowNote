...
현재 FastAPI 코드는 131건을 수집·통과하지만 표준 스크립트는 128건을 강제해 불일치한다. 스크립트를 131건으로 갱신한 뒤에도 최신 개발 호스트에는 Windows/.NET/JDK/Android SDK가 없어 WPF Core·앱 build·누적 스모크·Android build를 한 run으로 완료할 수 없다. 새 Windows 무생략 `verification-summary.json=PASSED`가 나올 때까지 마지막 통합 기준선 재확립은 `대기`로 본다.
