...
현재 코드와 표준 스크립트 guard는 FastAPI 160건·WPF Core 84건·Android 20건을 기준으로 한다. 이번 macOS 직접 실행에서 WPF Core 84/84가 통과했다. Windows 수집 목록과 원시 TRX의 `total/passed=84/84`, 누적 공통 DB 스모크와 Android build를 같은 clean 소스 커밋에서 새 run ID로 2회 완료해 각각 `partial_run=false`, `verification-summary.json=PASSED`가 나오기 전까지 통합 기준선 재확립은 `대기`다.
