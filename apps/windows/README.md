...
현재 FastAPI 코드는 154건, WPF Core는 76건을 기준으로 한다. 이번 macOS 검증에서 WPF Core 76/76과 WPF 앱 교차 빌드는 경고·오류 없이 통과했다. 표준 스크립트 guard는 FastAPI 154건·WPF Core 74건·Android 20건이므로 WPF Core 기대값이 현재 코드보다 2건 적다. guard를 바로잡고 Windows 수집 목록과 원시 TRX의 `total/passed=76/76`, 누적 공통 DB 스모크와 Android build를 같은 clean 소스 커밋에서 새 run ID로 2회 완료해 각각 `partial_run=false`, `verification-summary.json=PASSED`가 나오기 전까지 통합 기준선 재확립은 `대기`다.
