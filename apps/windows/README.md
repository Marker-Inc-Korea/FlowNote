...
- 현재 서버 API 클라이언트: Python FastAPI 서버의 로그인, 문서 등록/목록/버전 조회, FieldNote 등록, 문서 접근 로그 REST API 호출
...
- 스모크 테스트를 포함한 프로그램 테스트는 앱 로컬 공통 SQLite인 `apps/windows/src/FlowNote.Windows.App/Data/flownote.local.sqlite`를 계속 사용한다. 테스트마다 임시 SQLite를 새로 만들지 않고, 누적된 데이터와 이력을 보존한 상태에서 추가 검증한다.
