...
public final class OutboxStatusMessageTest {
    ...
    public void emptyQueueConfirmsServerStorage() {
        assertEquals(
                "전송 대기 0건 · 모든 현장 기록을 서버에 저장했습니다.",
                OutboxStatusMessage.format(
                        new OutboxQueueStatus(0, 0, 0, 0L),
                        1_000L,
                        true,
                        "tablet-a"
                )
    ...
    public void waitingQueueShowsPreservationAndNextRetry() {
        String message = OutboxStatusMessage.format(
                new OutboxQueueStatus(2, 0, 0, 76_000L),
                1_000L,
                true,
                "tablet-a"
        );

        assertTrue(message.contains("전송 대기 2건"));
        assertTrue(message.contains("암호화해 저장"));
    ...
    public void rejectedSessionExplainsLoginAndAdministratorContact() {
        String message = OutboxStatusMessage.format(
                new OutboxQueueStatus(1, 1, 0, 0L),
                1_000L,
                false,
                "tablet-a"
        );
        ...
        assertTrue(message.contains("대기 건수"));
    }

    @Test
    public void exhaustedQueueRequiresManualRetry() {
        String message = OutboxStatusMessage.format(
                new OutboxQueueStatus(3, 0, 2, Long.MAX_VALUE),
                1_000L,
                true,
                ""
        );
        ...
        assertTrue(message.contains("재전송을 누른"));
