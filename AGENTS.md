...
배포 기준은 고객 데이터 주권이다. 생산현장 문서는 클라우드 사용을 꺼리는 경우가 많으므로 내부망 로컬 서버, 고객 사내망 서버, 인가된 외부 접근 가능한 전용 서버, 고객 승인 전용 클라우드 등 설치 방식은 고객 보안 정책과 협의에 따라 결정한다. FlowNote는 특정 배포 위치를 강제하지 않는다.
...
현장 사용자는 로그인 기반 권한으로 문서를 열람한다. 파일 다운로드 차단과 뷰어 자동 닫힘은 중요 보안 요구이지만, 1차 Web/API 구현 범위가 아니라 Android/Windows 클라이언트 앱 단계에서 구현한다.

## Repository Layout

- `docs/`: 제품, 아키텍처, 데이터 모델, API, 결정 기록
- `services/api/`: FlowNote API 서버
- `apps/windows/`: WPF + WebView2 기반 관리자/데스크톱 클라이언트
- `apps/android/`: Kotlin + WebView 기반 현장 단말기 클라이언트
- `apps/web/`: TypeScript + React + Vite 기반 웹 UI
...
- Android는 Kotlin, Windows는 WPF를 기준으로 한다.
- Web은 TypeScript + React + Vite를 기준으로 한다.
- Database는 MySQL을 기준으로 한다.
- 배포는 고객 데이터 주권과 독립 인스턴스를 기준으로 하되, 실제 설치 위치는 고객 보안 정책과 협의에 따른다.
...
- Android와 Windows는 WebView 기반 UI를 우선하고, 파일 감시 같은 로컬 기능은 네이티브 브릿지로 처리한다.
- 고객 생산공장 운영에서는 일반 브라우저 직접 접근이 아니라 승인된 클라이언트 앱의 WebView 접근을 기본으로 한다.
