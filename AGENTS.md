...
- Windows 앱과 Windows 스모크 테스트는 앱 로컬 공통 SQLite인 `apps/windows/src/FlowNote.Windows.App/Data/flownote.local.sqlite`를 함께 사용한다. 스모크 테스트마다 임시 SQLite를 새로 만들지 않고 공통 DB에 기록과 이력을 누적한다.
...
- 사용자가 직접 테스트하거나 에이전트가 스모크 테스트를 실행하면서 생성한 SQLite 외 파일은 로컬 보존 대상이며 Git에 절대 올리지 않는다. PDF, 이미지, Excel, TXT, 렌더링 이미지, 테스트 로그, 테스트 등록 메모, `Data/Files/` 하위 파일은 Git 제외 테스트 산출물로 간주한다.
