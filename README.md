...
> **현재 상태:** 실제 운영 흐름을 구현하고 검증하는 연구 프로토타입이다. 아래 기능 설명은 2026-07-26 저장소 코드 기준이다. 운영 배포가 확정된 완제품은 아니다.
...
지금 가장 중요한 과제는 현장의 목소리를 꾸준히 취득하고, 서로 다른 요구를 종합해 제품 구조에 반영하는 일이다. 기능 하나를 개발하는 일보다 관찰과 검증에 더 많은 시간이 든다.
...
2026-07-26 현재 서버·Windows 컴포넌트와 Android 단위 테스트·debug 빌드를 다시 실행했다. Android lint 결과는 2026-07-24 검증 기록을 유지한다.
...
이 결과는 macOS ARM64 개발 호스트에서 실행한 API·Core 테스트, Windows 대상 교차 빌드, Android 개발 빌드를 기준으로 한다. 실제 Windows UI 조작, 공통 누적 SQLite를 사용하는 Windows 통합 스모크, 승인 Android 실단말, 운영 HTTPS·코드 서명·MDM 검증을 한 실행 ID로 묶은 현장 기준선은 아직 `대기`다. 상세 실행 기록과 과거 실패 증거는 [검증 자동화 문서](./docs/verification.md)에 보존한다.
...
전체 Windows 기준선은 `scripts/verify-preserved-tests.ps1`로 FastAPI, WPF, Android, 누적 SQLite 무결성과 스모크 증거를 하나의 실행 ID에 묶어 검증한다. 테스트 DB, 로그와 산출물은 회귀 분석 근거로 로컬에 계속 보존하되 Git에는 포함하지 않는다.
