...
function Get-StepExpectedValue {
    ...
    switch ($Name) {
        ...
        "Run WPF Core tests" {
            return "WPF Core 수집/고유 ${expectedWpfCoreTestCount}/${expectedWpfCoreTestCount}, TRX total/passed ${expectedWpfCoreTestCount}/${expectedWpfCoreTestCount}, 실패·오류·건너뜀 0건"
        }
        "Build WPF app" {
