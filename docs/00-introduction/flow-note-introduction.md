...
FlowNote는 고객 데이터 주권을 기준으로 한다. 내부망 로컬 서버, 고객 사내망 서버, 전용 서버, 전용 호스팅, 고객 승인 전용 클라우드 등 실제 설치 방식은 고객 보안 정책과 협의해 결정한다.
...
FlowNote의 Web UI는 TypeScript + React + Vite 기반으로 작성한다. Android 클라이언트는 Kotlin + WebView, Windows 클라이언트는 WPF + WebView2를 기준으로 한다.

실제 현장 운영에서는 일반 브라우저 직접 접근보다 승인된 클라이언트 앱의 WebView 접근을 기본으로 한다.
