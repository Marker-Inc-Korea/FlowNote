...
Android 현장 단말 앱, Windows/Android 공통 채널 알림, 인수인계 알림은 제품 방향에 포함하지만 현재 코드 기준 MVP 구현 완료 범위는 아니다.
...
Windows와 Android의 업무 채널 알림, 인수인계 확인, FieldComment/작업순서 이벤트 알림은 제외 범위가 아니다. 이 기능은 개인 메신저가 아니라 문서와 현장 기록에 연결되는 업무 이벤트 흐름으로 후속 MVP 후보에 포함한다.
...
- 서버-WPF 동기화 정책 고도화
- Android 현장 단말 앱
- Windows 채널 수신함, 채널 관리, 인수인계 확인 현황 화면
- 라인/설비/공정/작업조/인수인계 기준 공통 채널 알림
- Android 인수인계 작성, 수신 확인, 후속 FieldComment 연결
- 관리자 세션/계정 운영 UI
