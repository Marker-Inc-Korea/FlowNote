...
이 문서는 2026-08-15 현재 `ai_search` API, WPF `AI 정답셋`, 시드·검증 스크립트 구현 기준이다. 실제 현장 데이터셋·승인자·provider 심사가 아직 없는 부분은 운영 착수 조건으로 구분한다. 이 기반은 실제 AI 사용자 기능의 완료 판정에 포함하지 않는다.
