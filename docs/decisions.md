...
- 앱 화면과 foreground service는 같은 outbox의 전송 가능 항목을 15초 주기로 확인한다. 화면에는 전체 대기 건수, 다음 자동 재시도 시점, 자동 재시도 한도 초과 건수와 승인 단말 ID를 표시하며, 수동 `재전송`은 자동 대기 시간과 시도 한도를 넘겨 즉시 다시 시도한다.
