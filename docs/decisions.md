...
- `verification-summary.json`은 소스 커밋, FastAPI·WPF Core·Android unit 수집/실행 수, WPF·Android build, 공통 DB 전후 무결성, 오늘 문서와 과거 문서 version 증가, Git 전후 상태를 구조화해 남긴다. 2026-07-27 현재 수집값은 FastAPI 154건·WPF Core 71건·Android 16건이다. 표준 스크립트 guard의 WPF Core 기대값은 67건이므로 현재 수집값과 TRX 수를 확인해 갱신해야 한다. Windows x64 무생략 실행 2회가 끝나기 전에는 통합 기준선으로 확정하지 않는다.
