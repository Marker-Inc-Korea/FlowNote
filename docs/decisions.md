...
## 2026-06-30. FieldComment terminology

- The field-user feedback domain term is `FieldComment`.
- Avoid field-record names that visually compete with the product name `FlowNote`.
- New installations and final product code use the `field_comments` schema directly; no separate legacy field-comment compatibility path is kept for pre-release naming changes.
...
- WPF does not silently auto-refresh yet. When document, FieldComment, attachment, or access-log sync receives `401`, the local save remains, the sync queue row is marked failed, and the user-facing status tells the user to sign in again.
...
- 문서, FieldComment, FieldComment 첨부, 문서 접근 로그는 서버 전송 후보를 `server_sync_queue`에 남긴다.
...
- idempotency key는 문서 `wpf:document:{localDocumentId}:v1`, FieldComment `wpf:field-comment:{commentId}`, FieldComment 첨부 `wpf:field-comment-attachment:{attachmentId}`, 접근 로그 `wpf:access-log:{localAccessLogId}:{register_access_log_*}` 형식으로 생성한다.
- FastAPI는 문서 등록, FieldComment 등록, 문서 접근 로그 등록에서 선택 `idempotencyKey`를 받는다. 같은 키가 이미 있으면 새 행을 만들지 않고 기존 응답을 반환한다.
- FieldComment 첨부 API는 아직 서버 idempotency key를 받지 않는다. 현재는 WPF 로컬 큐의 고유 키와 `server_attachment_id`/`synced_at` 확인으로 중복 전송을 막고, 서버 첨부 idempotency는 후속 확장 대상으로 둔다.
...
- `team-member`와 `viewer`는 FieldComment 작성 중심으로 제한한다.
...
## 2026-06-24. FieldComment와 문서 버전 분리

- 현장 코멘트는 문서 파일 개정이 아니므로 신규 WPF 코멘트 저장 기본 경로는 `document_versions`가 아니라 `field_comments`이다.
- `document_versions`는 문서 파일 등록/개정 이력으로 유지한다.
- 기존 로컬 DB의 `document_versions.comment` 데이터는 초기화 시 `field_comments`로 백필한다.
- FieldComment는 원문 `raw_content`, 관리자 정리 `normalized_content`, 관리자 분석 `analysis_content`를 분리해 저장한다.
- 사진/파일 첨부는 `field_comment_attachments`와 `file_objects` 또는 WPF 로컬 첨부 테이블에 분리 저장한다.
...
- 최종 보고서는 다시 문서로 저장하고 원천 FieldComment, 작업내역, 관련 문서와 추적 가능하게 연결한다.
