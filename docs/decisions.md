...
- Android 앱은 승인된 현장 태블릿 또는 러기드 단말에서 문서 열람, FieldComment, 사진 기록, 신호등식 상태 기록, 인수인계, 채널 알림을 담당한다.
...
- Windows와 Android 전경 상태는 기본 15초 주기로 확인한다. 연결 실패 시 30초, 60초, 최대 120초까지 지수 백오프하고 성공 즉시 15초로 복귀한다. 서버 또는 네트워크 복구 뒤에는 마지막 cursor부터 재개한다.
- HTTP 401이면 polling을 중지하고 재로그인을 요구한다. 새 인증 세션은 저장한 사용자별 cursor부터 다시 시작한다. 다른 사용자 cursor를 공유하지 않는다.
