...
이 문서는 2026-08-15 현재 코드와 유효한 결정을 함께 기록한다. 대체된 결정은 현재 동작으로 오해되지 않도록 대체 사실만 남긴다.
...
- 미검토, 상충, 안전·품질 위험, 보고서 미연결과 담당자 없음 수는 WPF 로컬 복제본이 아니라 FastAPI가 `field_comments`와 `report_sources`의 현재 상태에서 집계한다.
- 검토 대시보드는 FieldComment 분석 권한 역할만 조회한다. 각 항목은 담당 역할, 다음 조치와 WPF 작업함 필터를 함께 반환한다.
- WPF는 같은 화면에서 AI 준비도 API도 읽지만 고객 승인을 받은 `ANONYMOUS_FIELD / FIELD_READINESS`와 `SYNTHETIC`·`TEST / SMOKE_REGRESSION`을 별도로 표시한다. 합성·시험 수치는 실제 현장 준비도나 부족 건수에 더하지 않는다.
- 두 서버 조회 중 하나라도 실패하면 로컬 또는 합성 수치로 빈자리를 채우지 않고 실제 서버 집계가 없다고 표시한다.
...
- `verification-summary.json`은 소스 커밋, FastAPI·WPF Core·Android unit 수집/실행 수, WPF·Android build, 공통 DB 전후 무결성, 오늘 문서와 과거 문서 version 증가, Git 전후 상태를 구조화해 남긴다. 2026-07-27에는 FastAPI 154건·WPF Core 71건·Android 16건을 수집했지만 표준 스크립트의 WPF Core 기대값은 67건이었다. 2026-07-28에 WPF Core 수집 목록과 TRX의 `total/passed=71/71`을 대조하고 기대값을 71건으로 갱신했다. 이후 시작 실패 안내 1건과 인수인계 후속 코멘트 멱등·부분 성공 2건을 추가해 macOS에서 74/74를 확인하고 기대값을 74건으로 맞췄다. 2026-07-30에는 AI 현장 표본 검토 테스트 2건을 포함한 수집 목록과 TRX `total/passed=76/76`을 다시 대조해 기대값을 76건으로 갱신했다. 후보 3의 사용자 오류 문구 순서와 부분 성공 재시도 대상 테스트 8건을 추가한 뒤 macOS 직접 실행 84/84를 확인하고 기대값을 84건으로 맞췄다. 실패 요약에는 현재 단계, 기대값, 실제값, 중단 원인, 보존된 데이터, 재실행 전 조치와 증거 경로를 함께 남긴다. Windows 수집 목록과 TRX 대조를 포함한 x64 무생략 실행 2회가 끝나기 전에는 통합 기준선으로 확정하지 않는다.
...
이 결정의 서버 계정 관리 경로는 [2026-07-14 서버 계정 수명주기 API와 Windows 운영 화면](#2026-07-14-서버-계정-수명주기-api와-windows-운영-화면) 결정으로 대체되었고, 로컬 로그인 허용은 2026-08-09 운영 HTTPS 단일화 결정으로 다시 대체되었다. 로컬/서버 데이터 분리와 서버 role 우선 원칙만 유지한다.
...
이 결정의 서버 계정 API 보류 범위는 2026-07-14 결정으로 대체되었고, 연결 실패 시 로컬 fallback 허용은 2026-08-09 운영 HTTPS 단일화 결정으로 대체되었다. 서버 role 우선과 모든 서버 오류에서 fallback 금지 원칙만 유지한다.
...
이 결정의 서버 계정 수명주기는 유지한다. 서버 미연결 로컬 로그인 화면을 운영 경로로 둔 부분만 2026-08-09 운영 HTTPS 단일화 결정으로 대체되었다.
...
- 계정 생성과 비밀번호 재설정은 운영자가 입력한 8자 이상 200자 이하 임시 비밀번호를 요청에서만 받고 `must_change_password = true`로 저장한다. 임시 비밀번호는 응답, 일반 로그, 활동 이력에 재노출하지 않는다.
...
이 결정은 2026-07-01과 2026-07-06 결정 중 서버 연결 실패에서 로컬 계정 fallback을 허용한 항목을 대체했다. 서버 URL 미설정 시 로컬 로그인을 허용한 나머지 부분은 2026-08-09 운영 HTTPS 단일화 결정으로 다시 대체되었다.
...
- 재시도 처리 순서는 문서 등록, 문서 버전, 문서 공개, 문서 상태, 문서 태그, FieldComment, FieldComment 검토, FieldComment 첨부, 접근 로그 시작/종료/자동 종료/다운로드 차단, 보고서 저장이다.
...
- Android 앱은 승인된 현장 태블릿 또는 러기드 단말에서 공개 문서 목록·상세 메타데이터 확인, FieldComment, 사진 기록, 신호등식 상태 기록, 인수인계 확인, 채널 알림을 담당한다. 당시 1차 범위에는 문서 파일 본문 뷰어와 인수인계 신규 작성 화면을 넣지 않았으며, 본문 뷰어는 2026-07-16, 인수인계 작성은 2026-07-30 별도 결정으로 추가했다.
...
- 앱 화면과 foreground service는 같은 outbox의 전송 가능 항목을 15초 주기로 확인한다. 화면에는 전송 완료·대기·실패를 서로 다른 아이콘과 한글 상태명·건수로 표시하고, 다음 자동 전송 시점, 자동 재시도 한도 초과 건수와 승인 단말 ID도 안내한다. 수동 `실패 항목 다시 보내기`는 로그인 상태의 `FAILED` 항목만 기존 멱등키로 즉시 다시 보낸다. 신규 `PENDING`과 완료된 `SYNCED` 항목은 선택하지 않으며, 자동 전송은 재시도 시점에 맞춰 `PENDING`과 `FAILED`를 처리한다.
- 로그인 만료, refresh 거부나 단말 비활성화 때도 outbox와 첨부를 지우지 않는다. 운영 패키지를 되돌릴 때는 후보 앱이 실제 단말에서 보고한 대기 0건 원시 로그를 먼저 보존하고, 대기 항목이 있거나 상태를 읽지 못하면 이전 APK 설치를 중단한다.
...
- Android 로컬 저장은 장기 원천 DB가 아니라 FieldComment, 사진 첨부와 신규 인수인계 재전송용 outbox로 제한한다. 신규 인수인계 확장은 아래 2026-07-30 결정을 따른다.
...
## 2026-07-30. Android 인수인계 작성은 원천 필수·멱등 outbox 계약으로 추가

- 현장 사용자는 자신이 속한 활성 업무 채널과 그 채널의 활성 수신자를 고른다. 작성 화면은 작업순서 항목, 공개 문서, 보관되지 않은 FieldComment, 보관되지 않은 작업내역 중 하나의 원천 ID를 필수로 요구하며 자유 대화만 저장하는 경로를 제공하지 않는다.
- Android 요청은 `entrySource = android_field_terminal`, 현재 승인 세션과 같은 `deviceId`, `android:{deviceId}:handover:{localId}` 멱등키를 보낸다. 서버는 단일 고객·현장 scope, 작성자 채널 멤버십, 수신자 채널 멤버십과 원천 가시성을 다시 검사한다.
- 서버는 같은 멱등키·같은 요청의 재전송에 최초 인수인계를 반환한다. `handovers`, `channel_messages`, `handover_receipts`는 한 transaction에서 생성하며 응답 유실·동시 재요청 뒤에도 중복 row 허용치는 0건이다. 같은 키를 다른 요청에 사용하면 409로 거부한다.
- 기존 Android outbox version 1의 `kind`, 암호화 JSON `payload`, `idempotency_key`, `server_id`를 그대로 사용해 `handover` 종류를 추가한다. DB 열 변경이 없으므로 기존 FieldComment 암호문과 APK rollback 읽기 계약을 바꾸지 않는다. 운영 rollback은 종전처럼 실제 단말 대기 0건 증거가 있어야 한다.
- 마지막 서버 확인 채널·수신자 목록은 서버 URL+사용자 scope별 Keystore 암호문으로 보관해 재부팅 직후 단절 중에도 작성할 수 있게 한다. 서버는 전송 시 멤버십과 원천을 다시 검사하며 로그아웃·단말 거부 때 선택 캐시는 삭제하고 outbox는 유지한다.
- FieldComment 사진은 `android-photo:{localId}` 첨부 멱등키로 재전송한다. FieldComment 저장 뒤 사진이 남으면 부분 성공으로 표시하고 같은 `comment_id`와 멱등키로 첨부만 재시도한다. 전송 완료한 앱 내부 암호화 사진은 다음 시작까지 잔존하지 않도록 정리한다.
- 저장 성공 뒤 사진 선택·미리보기·FieldComment 내용·신호등 선택과 인수인계 제목·본문·원천 입력을 비운다. 주요 버튼·수신자 선택은 56dp 이상이고 전송·보존·작성 상태는 접근성 live region으로 알린다.
- 로그인 만료, 권한 부족, 단말 비활성화, 재전송 대기, 부분 성공과 Keystore 오류는 보존 여부와 다음 행동을 구분해 한글로 표시한다. Keystore 오류에서는 입력·전송을 차단하고 재설치·초기화 대신 관리자 단말 교체 점검을 요구한다.
- Doze, 재부팅, 강제 중지 뒤 MDM kiosk 재실행, cursor·receipt 중복, 장갑·한 손·거치 조건의 성공률·시간·도움 요청은 코드와 가상 단말만으로 완료 판정하지 않는다. 승인 실단말, 서명 패키지와 같은 `run_id`의 원시 증거가 모두 통과해야 운영 완료다.

## 2026-08-03. Android 인수인계 확인·보류와 후속 코멘트도 보존형 흐름으로 처리

- 받은 인수인계의 확인은 `ACKNOWLEDGED`, 보류는 `FOLLOW_UP_REQUIRED`로 제한하고 서버에 보내기 전에 기존 암호화 outbox에 저장한다. 같은 receipt 공개 ID와 payload를 다시 보내도 서버 상태 변경 이력은 늘어나지 않는다.
- 후속 FieldComment는 인수인계 ID, 작성자와 정리한 본문의 SHA-256으로 안정된 `handover-follow-up:{digest}`를 만든다. 코멘트 본문에는 원천 인수인계와 업무 원천 ID를 함께 남긴다.
- FieldComment 저장과 채널 알림은 두 API 호출이므로 코멘트 성공 뒤 `comment_id`를 outbox row에 즉시 보존한다. 알림이 실패하면 부분 성공으로 표시하고 다음 재시도에서는 기존 코멘트로 `FIELD_COMMENT_EVENT`만 다시 보낸다.
- 작성 중 후속 입력은 서버 URL, 사용자와 인수인계 범위별 Keystore 암호문으로 저장한다. outbox 저장이 확인된 뒤에만 입력란을 비우며 로그아웃이나 단말 거부만으로 작성 중 입력과 업무 outbox를 삭제하지 않는다.
- Android 알림은 사용자 scope와 공개 `message_id`의 유일 원장을 별도 SQLite에 둔다. 시스템 알림 표시 뒤 원장에 기록하고 cursor를 전진시켜 cursor 재조회 때 같은 알림 표시를 반복하지 않는다.
- Windows 인수인계 확인 현황은 활성 채널을 함께 조회해 운영 단위와 채널을 표시한다. `UNREAD`, `READ`는 미확인 인원으로, `FOLLOW_UP_REQUIRED`는 후속 조치 인원으로 집계하며 receipt 원문은 그대로 유지한다.
...
## 2026-07-31. AI 민감정보 정책은 불변 버전과 분리 승인으로 운영

- 민감정보 정책 원문은 고객·현장별 불변 버전으로 저장한다. 변경이 필요하면 기존 원문을 수정하지 않고 새 버전을 작성한다.
- 상태는 `DRAFT → REVIEWED → APPROVED → ACTIVE`로 전이하며 작성자·검토자·승인자는 서로 달라야 한다. 활성 정책 대체, 승인 철회와 폐기 이력을 별도 상태로 보존한다.
- 모든 작성·상태 변경은 안정된 operation key, 최신 `stateTag`, 작업별 명시 확인값과 서버 read-back을 사용한다. 같은 요청의 재시도는 상태와 감사를 중복 생성하지 않는다.
- 목록·상세·감사·CSV에는 정책 원문과 고객 식별자를 반환하지 않고 content hash와 항목 수만 제공한다. 화면도 실제 고객·현장 식별값 대신 현재 적용 범위만 표시한다.
- provider 호출 직전과 응답 직후에는 정책 ID·content hash·revision snapshot을 다시 비교한다. 승인 철회·폐기 또는 snapshot 변경이 확인되면 신규 호출을 막거나 생성된 응답을 폐기한다.
- 질의 감사의 차단 사유와 실제 외부 전송 여부는 별도 값으로 제공한다. WPF는 `외부 호출 비활성`, `준비도 미달`, `정책 차단`, `kill switch`를 구분하고 담당자와 다음 행동을 표시한다.
...
- AFTER는 전건 성공, 치명적 blocker·권한 우회·원천 유실·처리 결과 유실·중복 생성 0건이어야 한다. 원천 보존과 다음 행동을 사용자가 모두 이해해야 한다. 중앙 완료 시간·화면 이동·도움 요청 중 하나 이상은 BEFORE보다 개선되고 이 세 지표와 중앙 재시도 수는 모두 악화되지 않아야 한다.
...
## 2026-07-30. UX BEFORE를 전체 파일럿과 분리해 판정

- schema version 9는 역할별 BEFORE 원시 행에 실행 ID, 실제 측정 상태·시각, 재시도, 기대·실제 결과, 원천 ID와 화면·감사 증거를 추가한다. 비관리자의 기대된 403은 권한 차단 성공으로 판정한다.
- 관찰별 개발 항목에는 영향 역할, 원천 보존 위험과 분류 근거가 있어야 한다. 한 현장의 선호를 근거 없이 공통 제품 변경으로 일반화하지 않는다.
- 운영·보안·현장 검토자는 모든 BEFORE 측정이 끝난 뒤 같은 실행 ID의 원시 승인표와 JSON 요약에 서명한다.
- `pilot-verification.json.ux_before_baseline`은 승인 계약, 역할별 BEFORE와 개발 항목 확정만 판정한다. 이 값이 `PASS`여도 최상위 `full_pilot` 결과와 배포 승인을 대신하지 않는다.
- 현재 run의 시간 한도 승인 시각이 통합 사전 승인보다 늦으면 실패 처리한다. BEFORE에서 계산해 사후 승인한 한도는 다음 run의 사전 승인값으로만 사용한다.

## 2026-07-30. Windows 생명주기와 망 전환은 무손실 원시 증거로 판정

- schema version 10은 framework-dependent와 self-contained MSI 각각의 신규 설치·업그레이드·제거·재설치·이전 승인본 rollback을 별도 행으로 요구한다. 각 단계는 승인 패키지 버전과 종료 코드 0뿐 아니라 WPF 로컬 데이터의 전후 fingerprint 일치까지 확인한다.
- 인증서 갱신·폐기와 서버 주소 변경은 기존 세션 요청, 로컬 로그인 자동 우회, 동기화 큐 처리와 알림 polling을 승인 복구 전까지 차단한다. 로컬 원천·큐·cursor는 유지하고 복구 후 중복 전송은 0건이어야 한다.
- HTTPS WPF client는 인증서 폐기 목록 확인을 사용한다. 폐기 확인이 실패하면 인증서 오류로 중단하며 HTTP 또는 로컬 계정으로 강등하지 않는다.
- 서버 재부팅 후와 승인 rollback 후에는 로그인, 문서 열람, FieldComment, 동기화, 알림, 감사 로그를 각각 다시 실행한다. 12개 원시 업무 행과 화면·WPF 로그·서버 감사 증거가 같은 `run_id`에 없으면 정상 업무 재개로 판정하지 않는다.
- 실제 Windows, 승인 서명 패키지, 고객 유사망과 인증서/CRL·방화벽·시간 변경 권한이 없는 개발 환경에서는 실행 도구와 판정 계약의 준비만 완료된 것으로 기록한다. 현장 실기 PASS를 추정하지 않는다.

## 2026-08-01. WPF 로그인 실패는 보존 상태와 복구 행동을 함께 안내

- 로그인 실패 화면은 한 줄 오류 대신 실패 내용, 누락 항목, 보존된 로컬 상태, 처리 담당자와 가능한 다음 행동을 구분해 표시한다. 서버 오류 원문이나 내부 경로는 노출하지 않는다.
- 로컬 문서·FieldComment·보고서 원천과 `Files`, 동기화 큐, 알림 cursor, 처리한 `message_id`와 마지막 정상 위치는 연결·인증서·로컬 저장소 시작 실패를 이유로 자동 삭제하거나 초기화하거나 덮어쓰지 않는다.
- 서버 URL이 설정된 상태에서는 주소 오류, 연결 실패, timeout, 인증서 chain·서버 이름·유효기간·폐기 확인 실패가 발생해도 HTTP나 로컬 계정으로 우회하지 않는다. 인증서 복구 안내에는 PC 시간, CRL/OCSP 접근과 사내 신뢰 배포 점검을 포함한다.
- 복구 안내는 키보드 초점이 가능한 읽기 전용 스크롤 영역에 표시한다. 다시 시도와 앱 종료 접근 키를 제공하고, 창 크기 조절과 버튼 줄바꿈으로 긴 문구와 높은 표시 배율에 대응한다.
- schema version 13의 `windows-startup-ux.csv`는 처음 화면을 본 참여자의 실패·원천/큐/cursor·담당자·다음 행동 식별, 키보드 전용 완료, 긴 문구 전체 접근, 200% 이상 배율과 버튼 겹침 0건을 요구한다. 자동 테스트 통과만으로 현장 접근성 PASS를 선언하지 않는다.

## 2026-08-03. Windows 문서 미리보기 실패는 앱 내부 보존과 공통 감사로 종결

- 현재 지원 범위는 Windows의 TXT, PDF, XLSX와 이미지다. HWP·CAD 고급 뷰어와 Android Office 렌더링은 이 변경에 포함하지 않는다.
- TXT는 UTF-8·UTF-16·CP949를 감지하고 긴 행과 긴 본문을 제한된 앞부분으로 표시한다. XLSX는 workbook 관계를 따라 최대 12개 시트와 시트당 100행·80열, 수식 캐시값·수식과 병합 범위를 읽는다. 이미지는 EXIF 회전과 지원 형식의 투명도를 유지하고 고해상도 미리보기만 최대 변 4,096px로 줄인다.
- PDF 암호·손상·부분 저장, 파일 접근 실패와 WebView2 시작 실패는 외부 앱이나 원본 복사로 우회하지 않는다. 모든 렌더러 실패는 파일 유형, 실패 범주, 원본·문서 버전·열람 이력·앱 세션 보존과 가능한 다음 행동을 표시한다.
- 사용자 화면과 실패 감사에는 기술 예외 원문, 고객 식별정보와 로컬 절대경로를 넣지 않는다. 실패 감사는 정해진 범주 코드만 사용한다.
- Windows의 열람 시작·종료·미리보기 실패·다운로드 차단은 로컬 접근 로그와 동기화 큐를 보존하고, 서버 도착 시 `document_access_logs`와 공통 `audit_event_envelopes`에 같은 사용자·세션·장비·문서·버전·correlation ID 문맥으로 기록한다.
- 자동 검증은 합성 익명 표본만 사용한다. 실제 익명 현장 표본은 사용 승인과 승인 환경이 확인된 별도 run에서만 검증하며 합성 결과와 합산하지 않는다.

## 2026-08-04 대형 변경 경로의 점진적 책임 분리

- 대형 파일은 줄 수 자체가 아니라 단일 책임과 의존 방향으로 판정한다. 새 파일과 이번에 수정한 production 파일은 1,000줄을 넘기지 않는다.
- 현재 수정 대상인 FieldComment 조회·검토·첨부, 보고서 source 검증·상태 전이, 작업순서 mutation·history부터 이동한다. `ai_search.py`, Windows `Program.cs`, `manage-pilot-run.py`와 대형 테스트 파일은 다음 기능 수정 때 책임별로 이동하며 이번 작업에서 일괄 재작성하지 않는다.
...
## 2026-08-09. 개발 PC의 서버 연동 검증은 운영 HTTPS로 단일화한다

- 이 결정은 서버 URL 미설정 시 로컬 로그인 또는 개발 PC의 로컬 FastAPI를 허용하던 2026-07-27 이전 결정을 대체한다. 과거 결정은 당시 동작 기록으로만 남긴다.
- Windows와 Android는 기본 운영 HTTPS 주소를 사용한다. override가 필요해도 승인된 HTTPS 주소만 허용하며 HTTP·localhost·잘못된 저장 주소로 우회하지 않는다.
- 개발 PC에서는 서버 코드를 작성하고 서버를 사용하지 않는 단위·정적 검증과 클라이언트 빌드를 수행할 수 있지만 FastAPI 프로세스를 띄우지 않는다. 서버 연동 기능·스모크·통합/E2E는 운영 HTTPS 서버에서 수행한다.
- 서버 변경이 필요한 검증은 현재 승인된 개발 PC의 저장소 밖 배포 권한으로 운영 서버에 먼저 반영하고 `운영 서버 테스트로 이관`을 명시한다. 배포 비밀값과 대상 정보는 코드·문서·Git에 기록하지 않는다.
- 운영 서버 DB·storage·로그 사본은 개발 PC에 남기지 않는다. WPF 로컬 작업 원장과 서버를 사용하지 않는 단위 테스트 산출물은 서로 다른 범위이며 기존 보존 정책을 따른다.
- 운영 서버 내부 reverse proxy가 사용하는 loopback FastAPI 바인딩과 내부 health 점검은 정상 서버 구성이다. 이것을 개발 PC의 로컬 FastAPI 실행으로 간주하지 않는다.
- 운영 HTTPS 서버가 응답하지 않거나 인증·인증서 검증에 실패하면 클라이언트는 로컬 계정 또는 HTTP로 fallback하지 않고 로컬 원천과 큐를 보존한 채 복구 안내를 표시한다.
