...
- 개별 검토는 `baseReviewRevision`과 `mutationKey`를 받아 `review_revision`을 조건부 증가시키고, 최초 응답 snapshot을 `field_comment_review_mutation_receipts`에 같은 transaction으로 보존한다.
...
- 첨부 재시도는 idempotency key와 부모 comment ID·요청 SHA-256·저장 파일 SHA-256을 검증해 응답 유실 뒤에도 첨부와 file object를 한 건으로 유지한다.
