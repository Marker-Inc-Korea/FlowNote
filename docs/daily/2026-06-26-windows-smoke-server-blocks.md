...
- 2026-07-28 현재 FastAPI 154건과 WPF Core 76건은 macOS에서 통과했다. 표준 통합 스크립트 guard는 FastAPI 154건·WPF Core 74건·Android 20건이므로 WPF Core 기대값을 현재 코드에 맞춰야 한다. 이후 Windows 수집 목록과 TRX의 `total/passed=76/76`을 대조한다. 과거 `baseline-131`과 `p0-baseline-144-macos-precheck-20260722-002`는 당시 일부 환경만 실행한 보조 기록이며 현재 통합 기준선이 아니다. 앱 빌드·공통 DB 스모크, Android 단위 테스트·debug build와 Git 산출물 사후 점검을 같은 실행 ID로 묶은 Windows x64 무생략 `PASSED` run을 새로 확보해야 한다.
