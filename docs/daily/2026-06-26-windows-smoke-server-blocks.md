...
- 2026-07-16 현재 FastAPI 코드는 126건을 수집하지만 표준 통합 스크립트의 고정 기준은 120건이다. 기준을 126건으로 맞춘 뒤 전체 pytest, WPF Core 테스트·앱 빌드·스모크, Android 단위 테스트·debug build와 Git 산출물 사후 점검을 같은 실행 ID로 묶어 다시 실행해야 한다.
