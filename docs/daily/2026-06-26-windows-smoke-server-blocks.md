...
- 2026-07-27 현재 FastAPI 154건과 Android 16건은 표준 통합 스크립트 guard와 일치하지만 WPF Core는 실제 71건보다 guard가 4건 적은 67건이다. 과거 `baseline-131`과 `p0-baseline-144-macos-precheck-20260722-002`는 당시 일부 환경만 실행한 보조 기록이며 현재 통합 기준선이 아니다. WPF Core guard를 현재 수집값과 TRX 수에 맞춘 뒤 앱 빌드·공통 DB 스모크, Android 단위 테스트·debug build와 Git 산출물 사후 점검을 같은 실행 ID로 묶은 Windows x64 무생략 `PASSED` run을 새로 확보해야 한다.
