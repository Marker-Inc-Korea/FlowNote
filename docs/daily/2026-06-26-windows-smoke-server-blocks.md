...
- 2026-07-30 현재 FastAPI 160건과 WPF Core 84건은 macOS에서 통과했다. Android 소스에는 단위 테스트 24건이 있다. 표준 통합 스크립트의 실제 고정값은 FastAPI 155건·WPF Core 84건·Android 24건이지만 FastAPI 단계 안내는 160건을 요구하므로, FastAPI 고정값과 안내를 현재 코드에 맞추기 전에는 수집 단계를 통과할 수 없다. 과거 `baseline-131`과 `p0-baseline-144-macos-precheck-20260722-002`는 당시 일부 환경만 실행한 보조 기록이며 현재 통합 기준선이 아니다. 스크립트를 바로잡은 뒤 앱 빌드·공통 DB 스모크, Android 단위 테스트·debug build와 Git 산출물 사후 점검을 같은 실행 ID로 묶은 Windows x64 무생략 `PASSED` run을 새로 확보해야 한다.
