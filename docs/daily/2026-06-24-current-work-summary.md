...
- 독립 2인 승인 ground-truth 사례, 불변 dataset version에 결합된 회귀 평가, 실제 익명 현장 24칸 독립 표본 검토·제3 합의, 외부 AI 질의의 기본 비활성·승인·근거 snapshot·응답 검증·인용 감사를 제공한다. `system-admin` 운영 제어는 민감정보 정책의 작성·분리 검토·승인·활성·대체·철회·폐기와 자동·일괄·단일 만료, legal hold 설정·해제를 포함한다. generic 네트워크 adapter는 명시적 test scope에만 있고 provider별 운영 client는 없다.
...
- 승인된 `FIELD_READINESS` dataset과 통과한 평가 run을 함께 선택하면 WPF `AI 정답셋`에서 24칸 독립 표본 검토와 불일치 제3 합의를 진행할 수 있다.
- `system-admin`은 별도 `AI 운영` 화면에서 승인·프롬프트·민감정보 정책·운영 정책·감사, 만료 보존 일괄·단일 즉시 실행과 legal hold 설정·해제를 관리한다. 민감정보 정책과 hold 관련 고위험 조작은 이중 확인, 상태 충돌 검사와 서버 read-back을 거친다.
...
- Android 현재 구현은 승인 단말 로그인, 공개 문서 목록·상세와 PDF/이미지/UTF-8 TXT 앱 내부 보안 열람, Keystore 보호 FieldComment/사진/인수인계 outbox, foreground service 채널 알림과 인수인계 작성·확인까지다.
