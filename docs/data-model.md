...
| change_type | BOARD_CREATED, ITEM_ADDED, ITEM_REORDERED, STATUS_CHANGED |
| before_value | 변경 전 값 |
| after_value | 변경 후 값 |
...
작업순서 변경을 즉시 알림으로 발송하기 전의 이벤트 후보를 저장한다. 현재 최소 구현은 순서 변경과 상태 변경 시 후보를 남기며, 실제 수신자 결정과 발송 정책은 후속 알림 정책에서 확장한다.
...
| event_type | work_sequence.reordered, work_sequence.status_changed 등 |
| actor_id | 수행자 |
| recipient_hint | 담당자 또는 라인 힌트 |
| message | 알림 후보 메시지 |
| status | CANDIDATE, SENT, DISMISSED |
| created_at | 생성일 |
