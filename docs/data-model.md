...
이 문서는 2026-07-21 현재 WPF `FlowNoteLocalDatabase`와 FastAPI `app/db/models.py` 기준이다. FieldComment 검토/첨부와 보고서 aggregate 수렴 필드는 구현되었으며, 현재 코드에 없는 나머지 필드는 `목표`로 명시한다.
