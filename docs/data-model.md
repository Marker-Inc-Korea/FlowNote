...
FlowNote의 메타데이터 DB는 MySQL을 기본으로 한다.

파일 바이너리는 MySQL에 직접 저장하지 않고 파일 저장소에 저장한다. MySQL에는 문서 메타데이터, 파일 참조 정보, 버전, 권한, 이력, 단말기, 현장 코멘트, 작업내역, AI 조언 로그를 저장한다.
...
| --- | --- |
| id | 내부 식별자 |
| storage_type | local, nas, object 등 |
...
문서 뷰어 열람 세션과 자동 닫힘 시간을 관리한다. 이 모델은 Android/Windows 클라이언트 앱 단계에서 사용하며, 1차 Web/API MVP의 실제 구현 범위에는 포함하지 않는다.
...
`FieldNote`는 최소 하나의 연결 대상이 있어야 한다. 1차 MVP에서는 `document_id` 또는 `structure_item_id`를 우선 사용하고, 작업내역 연결은 후속 단계에서 확장한다.
...
WebView 브릿지 호출 중 감사가 필요한 요청을 기록한다.

| 필드 | 설명 |
| --- | --- |
| id | 내부 식별자 |
| bridge_audit_id | 외부 참조용 브릿지 감사 ID |
| device_id | 단말기 ID |
| user_id | 사용자 ID |
| action | 브릿지 액션명 |
...
- 외부에는 `user_id`, `operator_id`, `session_id`, `document_id`, `structure_id`, `item_id`, `viewer_session_id`, `note_id`, `report_id`, `notification_id`, `work_record_id`, `advice_id`, `watched_file_id`, `candidate_id`, `bridge_audit_id`를 노출한다.
- 외부 참조용 ID는 변경하지 않는다.
- 물리 삭제보다 상태 변경을 우선한다.
