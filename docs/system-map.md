...
Windows WPF App
  ...
  -> channel inbox, handover management, notification review
...
WPF 앱에도 채널 수신함과 인수인계 관리 화면을 둔다. 관리자는 라인, 설비, 공정, 작업조, 작업내역 채널을 만들고 멤버를 관리하며, FieldComment와 작업순서 이벤트를 보고 인수인계 수신 확인과 후속 조치를 추적한다. WPF의 채널 기능은 기존 문서/보고서/검토 화면과 연결되는 감독 화면에 가깝고, Android의 빠른 현장 입력 화면과 같은 서버 데이터를 공유한다.

Android 앱은 현장 단말 입력을 우선한다. 현장 작업자는 공개 문서 열람, QR/검색 기반 문서 접근, FieldComment와 사진 기록, 신호등식 상태 기록, 인수인계 작성/확인, 채널 알림 확인을 수행한다. Android의 로컬 저장은 네트워크 불안정 구간의 임시 보관과 재전송을 위한 범위로 제한하고, 장기 기준 데이터는 FastAPI 서버에 남긴다.
...
Windows와 Android의 알림은 개인 메신저가 아니라 업무 채널 모델로 다룬다. 채널은 라인, 설비, 공정, 작업조, 작업내역, 인수인계 같은 운영 단위에 연결된다. 사용자는 자신이 속한 채널의 문서 공개/변경, FieldComment 등록/검토 요청, 작업순서 변경, 인수인계 등록/확인 요청 알림을 받는다.
...
초기 구현에서는 개인 DM, 사내 메신저 대체, 개인 휴대폰 알림 수집, GPS/근태 추적을 포함하지 않는다. Windows와 Android 알림은 서버 로그인 사용자, role, 채널 멤버십, 단말/클라이언트 승인 상태 기준으로 표시한다.
