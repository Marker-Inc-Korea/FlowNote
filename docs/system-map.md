...
현장 사용자는 문서 열람과 코멘트 등록 중심으로 사용한다. 관리자급 다운로드 role이 없는 사용자의 파일 저장/copy 시도는 Windows WPF 클라이언트 앱에서 차단하고 `download_blocked` 감사 로그로 남긴다.

문서 뷰어 자동 닫힘은 Windows WPF 클라이언트 앱에서 설정값 기준으로 적용한다. 이는 클라이언트가 켜져 있어도 사용자가 자리를 비운 상황에서 문서가 계속 노출되는 현실 보안 문제를 줄이기 위한 정책이다.
