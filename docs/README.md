...
- FastAPI 코드는 2026-07-22 현재 pytest node ID 144건을 중복 없이 수집하며 표준 스크립트 `scripts/verify-preserved-tests.ps1`의 수집/JUnit guard도 144건이다. macOS 보조 run `p0-baseline-144-macos-precheck-20260722-002`은 수집 144·고유 144·통과 144와 failure/error/skipped 0을 확인했지만 WPF·Android는 도구 부재로 `NOT_RUN`이다. Windows 무생략 `partial_run=false`, `verification-summary.json=PASSED` 기준선은 아직 확보해야 한다.
