...
- 현재 WPF는 서버 승인 작업함에서 최신 version·revision·file hash를 고정해 검토를 요청하고, 지정 검토자가 승인한 작업의 승인 ID로 공개한다. 요청·결정·공개·취소 이력은 서버에 보존하며 로컬 문서를 먼저 공개하거나 새 공개 동기화 큐를 만들지 않는다.
...
- WPF 서버 동기화 큐는 문서 최초 등록, 문서 버전, 문서 상태, 문서 태그, FieldComment, FieldComment 검토, FieldComment 첨부, 문서 접근 로그, 보고서 서버 저장을 대상으로 한다. FieldComment의 분석·검토 완료·보고서 선정은 각 상태 전이를 별도 큐 기록으로 보존한다. 보고서는 여러 문서의 근거를 묶더라도 나머지 전송 대기 항목을 먼저 처리한 뒤 시도한다. 상태·태그는 안정된 mutation key와 서버 receipt를 사용하고 2xx 응답 뒤 서버 문서 상태를 다시 읽어야 `SYNCED`로 종결한다. 누적된 구 `document_publish/publish_document_version` 큐와 처리기는 호환 이력으로 보존하지만 현재 UI는 새 공개 큐를 만들지 않으며, 승인 강제 기본값에서는 승인 ID가 없어 자동 공개할 수 없다. 작업내역 화면에서는 큐 깊이·최장 대기·최근 처리량·실패 분포와 row별 운영 상태를 확인한다. 서버본 유지로 종결한 `DISCARDED`도 전체 보존 건수에는 포함하되 재시도 대상 큐 깊이에서는 제외한다.
