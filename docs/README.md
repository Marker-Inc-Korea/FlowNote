...
- `scripts/manage-pilot-run.py`는 실제 파일럿의 단일 `run_id` 증거 구조와 schema version 3 판정표를 만든다. `full_pilot`과 `windows_server_rehearsal` 프로필을 분리하고, 8개 영역의 담당자/독립 승인자·시험 범위·중단 기준·증거 저장소, 통합 사전 승인, 익명 시험 장비, 이전 승인 버전을 먼저 검증한다. 선택한 프로필의 필수 게이트·0건 기준·rollback 업무 재개·운영/보안/현장 최종 승인을 실제 증거 상대경로와 함께 검증하며, 초기 판정표는 `PENDING`이고 실기 증거가 없으면 PASS를 만들지 않는다.
