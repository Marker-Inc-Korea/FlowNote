...
- FastAPI 코드는 2026-07-21 현재 pytest node ID 137건을 중복 없이 수집한다. 131건 기준 뒤 작업순서의 동일 key 재시도·stale revision 경쟁·API 재시작 receipt 회귀 3건과 FieldComment 검토 경합·첨부 응답 유실 재시도·보고서 source 변경 차단 회귀 3건이 추가되었다. 저장소 표준 스크립트 `scripts/verify-preserved-tests.ps1`의 guard는 아직 131건이므로 현재 코드와 일치하지 않으며, 137건으로 갱신한 뒤 Windows 무생략 `verification-summary.json=PASSED` 기준선을 새로 확보해야 한다.
