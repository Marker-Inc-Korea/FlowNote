...
- FastAPI 코드는 2026-07-22 현재 pytest node ID 145건을 중복 없이 수집하며 표준 스크립트 `scripts/verify-preserved-tests.ps1`의 수집/JUnit guard도 145건이다. macOS 사전 검증에서 145건 통과와 failure/error/skipped 0을 확인했지만 WPF·Android는 도구 부재로 `NOT_RUN`이다. Windows 무생략 `partial_run=false`, `verification-summary.json=PASSED` 기준선은 아직 확보해야 한다.
