...
이 폴더는 FlowNote의 제품 방향, 현재 구현, 데이터 모델, API, 보안, 배포 기준을 관리한다. 문서는 2026-07-10 현재 개발된 코드 기준을 우선하며, 아직 구현되지 않은 기능은 후속 범위로 분리한다.
...
- FastAPI 테스트는 2026-07-10 재수집 기준 58개다. 표준 PowerShell 검증 스크립트는 아직 53개를 고정 기대하므로 기대값을 갱신하기 전에는 전체 표준 검증 통과로 판정하지 않는다.
