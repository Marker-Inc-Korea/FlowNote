...
- FastAPI 코드는 2026-07-16 현재 pytest 128건이 수집·통과한다. AI 근거 검색·provider 경계·운영 제어·자동 보존, controlled copy, Android secure view, 서버 계정 수명주기·권한·세션·감사, 문서 revision·파일 hash 충돌, FieldComment 작업함·보고서 source 적격성 회귀가 포함된다. `scripts/verify-preserved-tests.ps1`의 수집/JUnit 고정값은 아직 120건이므로 현재 코드와 일치하지 않으며, 128건으로 갱신하기 전에는 표준 통합 검증이 이 단계에서 중단된다.
