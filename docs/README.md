...
- `scripts/manage-pilot-run.py`는 실제 파일럿의 단일 `run_id` 증거 구조와 schema version 6 판정표를 만든다. `full_pilot`과 `windows_server_rehearsal` 프로필을 분리하고, 8개 영역의 담당자/독립 승인자·시험 범위·중단 기준·증거 저장소, 통합 사전 승인, 익명 시험 장비, 이전 승인 버전을 먼저 검증한다. `full_pilot`은 Android 전달 8개 조건, 누락·receipt·crash 경계 무결성, 보안 8개 항목, 단말 수명주기 4개 시나리오와 후보·이전 승인 패키지를 원시 CSV로 대조한다. 역할별 UX는 성공률 95% 이상, 동일 시나리오 2회 반복, 화면 이동 수, 실제 관찰값 기반 시간 한도 승인과 모든 관찰의 수용/불수용/검토 결정을 원시 CSV로 대조한다. 수용한 P0/P1은 같은 개발 주기의 동일 역할·참여자·시나리오에서 수정 전후를 각각 2회 이상 측정하고 성공·중앙 시간·화면 이동·도움 요청이 퇴행하지 않아야 한다. `windows_server_rehearsal`은 서버/WPF 패키지 hash·signer·혼입, 설치/runtime matrix, 장애 주입, 별도 PC 복구, RTO/RPO와 rollback 후 6개 업무를 원시 CSV/JSON으로 대조한다. 모든 PASS 행의 증거 파일은 같은 실행 폴더에 실제로 있어야 하며, 초기 판정표는 `PENDING`이고 요약 JSON만 수정하거나 실기 증거가 없으면 PASS를 만들지 않는다.
