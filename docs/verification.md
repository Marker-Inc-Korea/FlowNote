...
`scripts/verify-preserved-tests.ps1`의 `$expectedFastApiTestCount`는 계속 131이다. 따라서 현재 코드 144건을 수집하는 표준 실행은 수집 단계에서 실패하며, guard 갱신 전에는 새 Windows x64 무생략 `PASSED` 기준선을 확정할 수 없다. 기존 `baseline-131-macos-precheck-20260721-001`은 당시 결과로 보존하지만 현재 코드 기준선으로 승격하지 않는다. 기존 실패 run, SQLite, 로그, 파일은 삭제하거나 초기화하지 않았다.
...
4. `services/api`에서 FastAPI pytest node ID 144개를 수집하고 중복 node ID가 0개인지 확인한다. 수집 목록은 `fastapi-collected-tests.txt`로 보존한다. 현재 스크립트 guard 131개를 144개로 먼저 갱신해야 이 단계가 통과한다.
