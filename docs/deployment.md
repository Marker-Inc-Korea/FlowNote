...
| 서버 | `FLOWNOTE_DOCUMENT_APPROVAL_WORKFLOW_ENFORCED` | 기본 `true`. 새 문서 공개에 정확한 version·revision·file hash 승인 근거 요구 |
| 서버 | `FLOWNOTE_DOCUMENT_APPROVAL_REQUESTER_REVIEWER_SEPARATION` | 기본 미설정. 같은 사용자가 요청·검토하는 현장 정책을 `true` 또는 `false`로 명시 |
| 서버 | `FLOWNOTE_DOCUMENT_APPROVAL_REQUESTER_PUBLISHER_SEPARATION` | 기본 미설정. 같은 사용자가 요청·공개하는 현장 정책을 `true` 또는 `false`로 명시 |
...
관리자 화면은 영문 판정 코드를 숨기지 않고 한글 의미와 데이터 영향을 함께 표시한다. `CONFIRMED`는 같은 idempotency 원천이 서버에 있음을, `ABSENT`는 서버 원천이 없음을, `DIVERGED`는 hash 또는 revision이 다름을 뜻한다. 이 원판정은 아직 상태를 바꾸지 않은 서버 관측 결과로 표시한다. 승인 조치 `REBOUND`는 서버 ID 매핑과 큐 완료 상태를 갱신하고, `REQUEUE`는 서버 매핑을 해제해 로컬 원천을 재전송 대기로 돌리며, `CONFLICT`는 자동 전송을 종결하고 양쪽 hash와 승인 사유를 보존한다. 각 행에는 로컬·서버 SHA-256과 서버 문서·버전 ID도 함께 표시한다. 화면 상단에는 상태·매핑 변경 대상, 원천 보존, 충돌 격리 건수와 승인 후 재시작 절차를 먼저 보여 준다. `승인 적용`은 일반 운영 동작과 분리된 위험 조치로 표시하며, 관리자 승인 사유와 영향·보존 범위·되돌릴 수 없는 승인 기록·재시작 절차를 읽었다는 확인이 모두 있어야 활성화된다. 마지막 확인 창에서도 run별 조합과 건수, 원천 보존 범위, 승인 후 재시작 조건을 다시 보여 준다.
