...
작업순서의 최종 운영은 서버 직접 API로 결정했다. 목표 계약에서 WPF 작업순서 화면은 서버 revision을 기준으로 보드·항목을 변경하고 서버가 같은 트랜잭션에서 이력을 만든다. 현재 WPF 로컬 작업순서 테이블과 화면은 아직 이 경계로 전환되지 않았으므로 오프라인 초안·읽기 캐시와 기존 테스트 기록으로 격하해야 하며 `server_sync_queue`에 새 작업순서 mutation을 넣지 않는다. 전환 뒤 서버 미연결 상태에서는 확정, 순서 변경, 상태 변경을 허용하지 않는다. FieldComment 검토, 첨부, 보고서와 source의 revision·idempotency·source snapshot도 아직 구현 전 목표 계약이며, 구현·migration·장애 주입 검증을 통과하기 전에는 두 WPF 인스턴스 동시 쓰기를 운영 승인하지 않는다.
