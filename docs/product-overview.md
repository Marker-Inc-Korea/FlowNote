...
현재 구현 설명은 2026-07-27 코드 기준이며 장기 제품 방향이나 아직 구현하지 않은 기능은 배포 방향·제외 범위에서 명시적으로 구분한다.
...
- AI 자동 조언 전 단계의 근거 검색 후보 재생성, 목록 조회, 품질 점검, scope별 ground-truth 첫 승인·독립 2차 승인, 실제 현장/스모크 준비도 분리, 불변 dataset version과 오프라인 회귀 평가, 실제 익명 현장 24칸 독립 표본 검토·제3 합의 서버 API, WPF 운영 점검·`AI 정답셋`·사례 원천 구성 화면. 표본 검토용 WPF 화면과 클라이언트는 아직 없음
