...
현재 구현 설명은 2026-08-06 코드 기준이며 장기 제품 방향이나 아직 구현하지 않은 기능은 배포 방향·제외 범위에서 명시적으로 구분한다.
...
서버와 WPF 동기화 큐는 문서 최초 등록, 문서 버전, 문서 상태, 문서 태그, FieldComment, FieldComment 검토, 첨부, 접근 로그, 보고서 서버 저장을 대상으로 하며, 서버 URL이 없거나 실패하면 로컬 저장을 우선한다. 재시도는 같은 문서/근거 단위로 묶은 뒤 문서 등록, 버전, 누적 구 공개 큐, 상태, 태그, FieldComment, 검토, 첨부, 접근 로그, 보고서 순서로 처리한다. 선행 서버 ID가 없으면 실제 서버 호출 없이 보류 사유를 남기고 로컬 데이터는 삭제하지 않는다. 현재 UI의 새 문서 공개는 동기화 큐가 아니라 서버 승인 작업함에서 직접 처리한다.
