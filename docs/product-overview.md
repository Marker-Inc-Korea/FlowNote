...
초기 제품은 AI 기능보다 데이터 축적과 현장 사용성을 우선한다. 현재 AI 관련 구현은 DB 원천에서 `ai_search_candidates` 근거 후보를 재생성·조회·품질 점검하는 API와 WPF 운영 점검 화면, 그리고 `/api/v1/ai/queries` 질의 생성·조회 및 외부 호출 전 안전장치 골격까지다. 질의 라우터는 기본 비활성 플래그, 관리자 role, 허용 목적, 고객·현장·provider·model 승인, 승인된 프롬프트, 근거 원천 상태와 응답 인용 ID를 검사하고 감사 row를 남긴다. 다만 저장소에는 운영 provider client와 네트워크 호출이 없고, 현재 주입 경계에는 질의/DB 원문이 아닌 hash·프롬프트 버전 ID·후보 ID만 전달하므로 실제 외부 AI 요약을 운영할 단계는 아니다. 후속 운영 연동도 “근거가 있는 검색과 요약”으로 제한하고 모든 사실 주장을 질의 시점 근거와 연결한다.
...
- Android 현장 단말 최소 앱: 승인 단말 로그인, 공개 문서 조회, FieldComment, 사진 첨부 outbox, 신호등식 기록, 전경 채널 알림 polling/읽음, 인수인계 확인
...
Android 현장 단말 앱과 Windows 채널/인수인계 전용 화면은 현재 최소 구현이 들어와 있다. 공통 채널과 인수인계 서버 모델/API는 FastAPI에 구현되어 있으며, Windows WPF는 관리자/현장 PC의 문서 운영, 파일 감시, 보고서 정리, 로컬 동기화 보강, 채널 감독, 인수인계 관리와 승인 단말 운영을 담당하고, Android는 승인된 현장 태블릿 또는 러기드 단말에서 문서 열람, FieldComment, 사진 기록, 신호등식 상태 기록, 인수인계 확인, 채널 알림 확인을 담당한다. Windows와 Android는 전경에서 기본 15초 간격으로 알림을 polling하고, 연결 실패 시 최대 120초까지 backoff한다. Android는 사용자별 마지막 cursor를 로컬 설정에 보존하고, WPF는 열린 주 창 세션의 cursor만 메모리에 유지하며 새 창 세션은 0부터 증분 목록을 따라잡는다. WPF 승인 단말 화면에서는 서버 단말의 목록·상세·마지막 접속을 조회하고 등록, 정보/상태 변경, 교체를 수행할 수 있다. 운영 배포 서명, MDM/인증서 적용, 현장별 단말 등록·비활성화 절차, 백그라운드 알림 정책과 채널 운영 UX 고도화는 후속 범위다.
