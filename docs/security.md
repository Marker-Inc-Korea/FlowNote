...
- 서버 `401`·`403` 응답 뒤 로컬 fallback은 `0건`이어야 한다. 허용된 서버 미설정·연결 불가 fallback은 별도 감사가 있는 경우에만 성공으로 센다.
...
| DNS/TCP 실패, 연결 거부, timeout | 사전 승인된 비상 조건에서만 로컬 로그인 허용 | 로컬 문서와 보존 큐를 사용하는 승인 업무. 서버 controlled copy, 계정·단말·AI 운영은 금지 | `local_fallback_server_unreachable`과 실패 종류, 서버 scope, 사용자·PC·승인번호를 남기고 연결 복구 뒤 서버와 대조한다. |
...
| TLS 인증서·호스트명·신뢰 실패, 응답 위변조 의심 | fallback 금지 | 없음 | 보안 사고 후보로 중단하고 인증서와 서버 주소를 확인한다. 일반 연결 불가로 축소하지 않는다. |

현재 WPF는 서버 URL 없음과 `HttpRequestException`·timeout에서 로컬 로그인을 시도한다. 따라서 인증서 오류를 일반 연결 실패와 구분하는지, 허용 fallback 자체가 사용자·PC·서버 scope·사유와 함께 감사되는지는 현장 역검증의 필수 항목이며 충족되지 않으면 구현 공백으로 판정한다.
...
- 실제 현장 평가의 독립 표본 API는 보고서 작성 role인 `admin`, `system-admin`, `document-admin`, `manager`, `assistant-manager`, `department-manager`에 허용한다. 표본은 24개 범주·유형 칸에서 1건씩 고정하고 두 검토자는 같은 표본을 독립 판정하며 서로 달라야 한다. 첫 판정의 `findings`와 `decisionHash`는 두 번째 검토 전까지 다른 사용자에게 숨긴다. 결과가 다르면 `PENDING_CONSENSUS`를 유지하며 앞선 두 사람과 다른 제3 검토자의 합의 기록 없이는 provider 착수 게이트를 통과하지 않는다.
