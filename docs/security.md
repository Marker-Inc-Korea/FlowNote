...
- 채널·인수인계 알림의 초기 전달은 외부 인터넷에 의존하지 않는 사내망 HTTPS polling이다. 서버는 Bearer token과 활성 채널 멤버십을 매 요청 검증한다. Android는 사용자별 마지막 cursor를 로컬 설정에 보존하고, WPF는 현재 주 창 세션의 cursor를 메모리에 유지한다. 읽음 변경은 공개 `message_id`를 멱등 키로 사용한다.
...
외부 AI 호출은 기본 비활성화하며 서버 설치나 provider 자격증명 등록만으로 켜지지 않는다. 현재 코드는 `FLOWNOTE_AI_EXTERNAL_CALL_ENABLED=false`를 기본으로 두고, 이 값을 켜더라도 설정의 고객·현장·provider·model과 일치하는 미만료·미철회 `ai_transfer_approvals` row가 없으면 차단한다. 승인과 프롬프트를 등록·철회하는 운영 API/UI는 아직 없으므로, 현재 테스트는 DB에 정책 row를 직접 준비한다. 후속 운영 절차에서는 고객·현장별 `admin` 또는 `system-admin`이 다음 항목을 확인한 뒤에만 승인 row를 생성하고 별도 기능 플래그를 켜야 한다.
...
현재 질의 생성 코드는 원천 상태와 승인된 source type을 재검사한다. 외부 전송 대상 FieldComment는 `ANALYZED`, `REVIEWED`, `SELECTED`로 제한하고, 문서는 삭제되지 않은 현재 공개 버전, 작업순서는 존재하는 변경 이력, 보고서는 비보관 보고서의 source row만 적격으로 선택한다. provider/model/system prompt는 서버 설정과 승인 프롬프트에서 선택하므로 사용자가 임의로 바꿀 수 없다. 현재 provider 주입 경계에는 질의·프롬프트·근거 원문이 아닌 hash·버전 ID·후보 ID만 전달된다.

운영 provider 연동 전에는 현재 사용자의 원천별 열람 권한, 금지 패턴과 분류 태그, 민감정보·대외비/수출통제 필터를 추가해야 한다. 실제 provider payload를 구현할 때도 전체 파일·사진·첨부가 아닌 필요한 최소 텍스트 조각과 불투명 근거 참조값만 전송하고 사용자명·로컬 경로·내부 URL을 제거해야 한다. 이 항목이 구현·검증되기 전에는 기능 플래그를 활성화하지 않는다.

질의 원문과 저장이 승인된 응답 본문은 제한 데이터로 취급한다. 현재 생성·조회 API는 `admin`, `system-admin`만 사용하지만, 조회 API는 질의 원문·응답 본문·citation 목록을 반환하지 않고 질의 상태·응답 hash·근거 snapshot만 반환한다. 또한 현재는 호출자 본인만 조회하도록 제한하지 않으므로 관리자 간 질의 조회 범위를 운영 provider 연동 전에 정해야 한다. 일반 서버/프록시 로그에는 질의, 프롬프트, 근거 본문, 응답, 자격증명, provider raw 오류를 남기지 않는다.

코드는 질의 원문과 응답 보관 기한 메타데이터를 90일로 설정하고, `DO_NOT_STORE`에서 응답 본문 대신 hash만 저장한다. `query_id`, 호출 사용자, 근거 후보/원천 ID, 프롬프트 버전, provider/model, 응답 저장 여부, 상태, 정제한 오류 코드와 시각은 DB row로 남긴다. 근거·인용·호출·오류·승인 메타데이터의 1년 보존, 만료 삭제 스케줄러와 삭제 감사 이벤트는 운영 정책이며 아직 구현되지 않았다.
