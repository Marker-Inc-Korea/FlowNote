...
현재 WPF는 서버 URL이 없을 때만 로컬 로그인을 시도한다. 서버 URL이 설정된 상태의 `HttpRequestException`·timeout은 로컬 로그인으로 자동 우회하지 않고, 인증서 오류는 PC 시간·운영 서버 이름·사내 인증서 신뢰 배포를, 일반 연결 오류는 네트워크·방화벽·현재 서버 주소를 확인하도록 안내한다. 사전 승인된 비상 fallback은 사용자·PC·서버 scope·사유·승인번호를 감사하는 별도 UI가 구현되기 전까지 자동 허용하지 않는다.
