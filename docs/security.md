...
현재 코드에서 실제 구현된 보안 기능은 Windows WPF 앱 시작 시 로그인 요구, 기본 계정 생성, 로그인 성공 후 화면 진입, 알림 대상 사용자 식별 수준이다. 다운로드 차단, 뷰어 자동 닫힘, 서버 감사 로그, 역할 기반 서버 권한, 외부 접속 제어는 아직 구현된 기능이 아니라 제품 보안 목표이다.
