...
이 문서는 2026-08-15 현재 코드에 적용된 통제와 운영 전 후속 통제를 구분한다.
...
- WPF 시작 시 로그인 요구
- WPF 운영 HTTPS 서버 계정과 활성 세션 확인. 로컬 계정 모델은 기존 데이터·단위 테스트 호환용으로만 보존
...
- 테스트 전용 비밀번호 `1234`로 서버 로그인이 401로 실패할 때 WPF가 기존 로컬 호환 계정으로 우회하지 않는지 확인한다.
...
| 작업순서 생성·순서·상태·보류 사유 변경 | 허용 | 허용 | 허용 | 허용 | 금지 | 서버는 문서 쓰기 role, 최신 board revision과 공백이 아닌 mutation key를 검사한다. `HOLD` 전환에는 `holdReason` 또는 `changeReason`이 필수다. 그 밖의 상태·순서 변경에서 `changeReason`은 선택값이므로 모든 변경에 필수 사유를 요구하는 현장에서는 구현 공백으로 기록한다. |
...
- 운영 HTTPS 서버의 `401`·`403`·연결 불가·timeout·TLS 오류 뒤 HTTP 또는 로컬 fallback은 `0건`이어야 한다.
...
## 서버 계정 표준 운영과 로컬 계정 호환 경계

서버 계정이 현재 유일한 표준 운영 로그인 경로다. WPF 로컬 계정은 기존 데이터·단위 테스트 호환을 위해 남아 있을 뿐 서버 미설정이나 연결 실패 시 사용할 수 없다. 같은 로그인 ID가 두 저장소에 있어도 동일 계정으로 간주하거나 자동 병합하지 않는다.

| 상태 | 로그인·fallback 판정 | 허용 범위 | 필수 조치와 감사 |
| --- | --- | --- | --- |
| 서버 URL 미설정·잘못된 주소·HTTP 주소 | 운영 기본 HTTPS 주소 적용 또는 시작 차단 | 없음 | 승인된 HTTPS 주소와 인증서를 확인한다. 로컬 FastAPI나 로컬 계정으로 우회하지 않는다. |
...
현재 WPF는 기본 운영 HTTPS 주소 또는 승인된 HTTPS override로만 로그인을 시도한다. HTTPS `HttpClientHandler`는 인증서 폐기 목록 확인을 켜며, 인증서·chain·호스트명·유효기간·폐기 확인에 실패한 연결을 우회하지 않는다. `HttpRequestException`·timeout에서도 HTTP나 로컬 로그인으로 자동 우회하지 않고, 인증서 오류는 PC 시간·운영 서버 이름·폐기 상태·CRL/OCSP 접근·사내 인증서 신뢰 배포를, 일반 연결 오류는 네트워크·방화벽·현재 서버 주소를 확인하도록 안내한다. 실패 화면에는 실패 내용, 보존한 로컬 원천·`Files`·동기화 큐·알림 cursor·처리 `message_id`, 처리 담당자와 가능한 다음 행동을 함께 표시한다. 로컬 저장소 시작 실패도 자동 초기화나 재설치를 먼저 실행하지 않고 DB와 `Files`의 동시 보존과 관리자 승인을 요구한다. 비상 fallback은 별도 감사 UI가 구현되고 사용자가 정책 변경을 명시적으로 승인하기 전까지 허용하지 않는다.
...
1. 현장 책임자가 운영 HTTPS 서버 주소, 인증서와 허용 PC·사용자를 승인한다.
2. 계정 운영자는 서버 계정을 발급하고 임시 비밀번호 변경을 확인한다. 로컬 계정을 새로 발급하지 않는다.
...
- PDF는 WebView2 기반 표시를 우선하고 저장·다운로드·새 창과 현재 PDF 이외의 이동을 차단한다.
- 텍스트, 이미지, Excel은 앱 내부 읽기 전용 미리보기로 표시한다.
- Windows 미리보기 실패 화면은 파일 유형, 실패 범주, 원본·버전·열람 이력·세션 보존과 가능한 다음 행동만 표시한다. 기술 예외, 고객 식별정보, 서버 오류 본문과 로컬 절대경로는 표시하지 않는다.
- 열람 시작·종료·미리보기 실패·다운로드 차단은 로컬 접근 로그와 재시도 큐를 보존하면서 서버 공통 `audit_event_envelopes`에 사용자·세션·장비·문서·버전·correlation ID 문맥으로 연결한다.
...
- 검토 요청은 문서 작성 role, 결정·취소·공개는 문서 governance role만 수행한다. 사용자 지정 요청은 활성 계정이며 governance role인 검토자만 허용하고, role 지정도 governance role만 허용한다.
- 요청자와 검토자, 요청자와 공개자의 분리 여부는 각각 현장 설정이다. 설정이 없는 상태에서 동일 actor 처리를 허용하거나 금지했다고 추정하지 않고 fail-closed로 차단한다.
