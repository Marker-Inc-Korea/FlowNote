...
2026-07-31 현재 저장소에는 리허설·파일럿 증거 생성과 판정을 보조하는 스크립트가 있지만 현장별 서명·인증서·MDM·고객 유사망·별도 PC 복구를 완료했다는 실기 증거는 없다. 따라서 이 문서는 구현 완료 목록이 아니라 현재 코드와 운영 보조 도구에 적용할 후속 실기 절차다.
...
| `previous_approved_versions.server`, `.wpf`, `.android` | 실제 복귀 가능한 패키지/백업 세트의 승인 버전 식별자 | 미확정 |
| `previous_approved_packages.server`, `.wpf`, `.android` | 이전 승인 버전, 패키지 SHA-256, signer 인증서 SHA-256 | 미확정 |
...
| `approvals.operations`, `.security`, `.field_operations` | 서로 다른 독립 승인자의 `PASS`, 시간대 포함 시각, 근거 참조와 실행 폴더 안의 원시 증거 | 미확정 |
...
`full_pilot`은 다음 순서로 시작한다. `prepare`는 단일 `run_id`의 `pilot-run.json`, `manifest.md`, 승인 원시표 3종과 `pilot-readiness.json`, `.csv`, `.html`을 만든다. 이 단계에서는 `install/`, `backup-restore/`, Android 운영 CSV를 만들지 않으므로 실기 입력이 잠겨 있다. 기존 실행은 기본적으로 덮어쓰지 않는다.

```powershell
py -3 scripts\manage-pilot-run.py prepare --run-id PILOT-YYYYMMDD-HHMM-SITE-001 --evidence-root D:\FlowNotePilotEvidence
```

`approvals/responsibility-assignments.csv`, `approvals/pilot-approval-signatures.csv`, `approvals/rehearsal-authorization.md`와 `pilot-run.json`에 승인값을 기록한 뒤 다음 명령을 실행한다. `authorize`는 8개 책임 영역의 담당자·독립 승인자·시각·근거 참조, 통합 시험 범위, 서로 다른 중단 기준 5개 이상, 공통 증거 저장소 식별자, 보존 기한, 승인 장비, RPO/RTO, rollback 권한, 비상 연락, 서버/WPF/Android 이전 승인 패키지와 운영·보안·현장 서명을 대조한다. 하나라도 빠지거나 원시표와 JSON이 다르면 운영 입력을 계속 잠근다.

```powershell
py -3 scripts\manage-pilot-run.py authorize --run-id PILOT-YYYYMMDD-HHMM-SITE-001 --evidence-root D:\FlowNotePilotEvidence
```

승인이 확정되면 `approvals/pilot-authorization-events.jsonl`에 현재 `run_id`와 계약 SHA-256이 `AUTHORIZED` 이벤트로 추가되고 그때 `scenario-results/android-delivery.csv`, `scenario-results/role-metrics.csv`, `scenario-results/role-ux-comparison.csv`, `scenario-results/restore-fault-injections.csv`, `observations/role-observations.csv`, `observations/development-items.csv`와 설치·복구·Android 운영 템플릿이 생성된다. 이미 있는 파일은 덮어쓰지 않는다. 익명 참여자 ID만 사용해 원시 관찰값을 적고 집계한 분모·성공·중앙값·최대 시간·재시도·도움 요청·화면 이동 수는 `pilot-run.json` schema version 12에 옮긴다. 최종 검증은 요약값을 신뢰하지 않고 역할 원시 행에서 다시 계산해 일치 여부를 확인한다. Android 전달 항목은 정상, Doze, 5분 단절, 재부팅, 서버 주소 변경, access token 만료, refresh 거부, 강제 중지 뒤 kiosk 재실행을 각각 전건 성공과 허용 시간 이내로 기록해야 한다. 정상·Doze의 `allowed_seconds`는 30, 5분 단절의 값은 `30 + page_seconds`로 고정하며 임의 완화할 수 없다. 나머지 시나리오도 사전 승인한 양수 허용 시간과 측정 최대 시간을 반드시 기록한다.

`pilot-readiness.html`은 미충족 항목을 역할·게이트·선행조건별 건수로 요약하고 각 행에 현재 담당 역할과 다음 행동을 표시한다. 같은 내용은 `pilot-readiness.json`과 `pilot-readiness.csv`에도 보존된다. 기존 판정표를 다시 묶을 때는 `readiness` 명령을 사용하며 `pilot-verification.json`이나 원시 증거를 수정하지 않는다.

```powershell
py -3 scripts\manage-pilot-run.py readiness --run-id PILOT-YYYYMMDD-HHMM-SITE-001 --evidence-root D:\FlowNotePilotEvidence
```
...
패키지 표에는 후보 서버 패키지, framework-dependent와 self-contained 후보 WPF MSI·EXE, 이전 승인 서버 패키지·WPF MSI가 각각 정확히 한 행 있어야 한다. hash와 signer는 승인값과 같고 서명·chain·timestamp가 PASS이며 비밀·SQLite·고객 파일 혼입 수가 모두 0이어야 한다. 설치 표에는 서버 신규 설치와 두 WPF MSI 각각의 신규 설치·업그레이드·제거·재설치·이전 승인본 rollback을 기록하고 단계 전후 로컬 데이터 fingerprint가 같아야 한다. runtime 표는 두 MSI 각각의 .NET Desktop Runtime 설치/미설치와 WebView2 설치/미설치를 기록한다. 시작 실패 UX 표는 관리자와 일반 사용자가 각각 4회씩 .NET 누락, WebView2 누락, 잘못된 서명·서버 주소, 인증서, 방화벽, 시간, 서버 자동 시작 오류 화면에서 누락 항목·보존 데이터·담당자·다음 조치를 식별하고 실제 선택한 조치를 기록한다.

장애 주입 표에는 작업 스케줄러, 재부팅 자동 시작, 인증서 갱신·폐기·검증 오류, 방화벽 포트 차단, DNS·고정 주소 변경, 시간 오차, 디스크 부족, 장기 네트워크 단절, 서버 재부팅 중 전송, 업그레이드 중단, 잘못된 패키지 서명이 각각 정확히 한 행 있어야 한다. 각 행은 장애 감지, 미승인 클라이언트 차단, 승인 클라이언트 재연결, 정상 업무 재개, 원천·동기화 큐 보존, 중복 전송 0건, 변경 승인 ID와 재개 시각을 모두 가져야 한다. 별도 네트워크 fail-closed 표는 인증서 갱신·폐기와 주소 변경의 기존 세션, 로컬 로그인 fallback, 동기화, polling 차단과 원천·큐·cursor 보존, 승인 복구 후 중복 0건을 각각 강제한다.
...
모든 사용자는 본인 계정으로 수행하고 성공 여부, 시작·종료 시각, 재시도 수, 도움 요청, 화면 이동 수, 오류 메시지를 기록한다. 대리 입력은 실제 전달자 또는 작업자와 대리 입력자를 함께 남긴다. 각 필수 역할·시나리오는 같은 조건에서 최소 2회 반복한다.

| 역할 | 필수 시나리오 | 성공 기준 |
| --- | --- | --- |
| 문서관리자(`document-admin`을 포함한 관리자 시나리오) | 계정·단말 운영, 문서 등록·버전·상태·공개, FieldComment 원천 불변 확인·담당/기한·단계형/일괄 검토·감사/품질 작업함, 보고서 source 선정·저장, 장애 로그 확인 | 승인된 기능만 보이고 모든 변경 주체와 사유가 추적됨 |
...
Windows 로그인 직후 표시하는 첫 업무 3개는 다음 순서를 기준으로 한다. 이 순서는 전체 필수 시나리오를 세 개로 축소하는 뜻이 아니라, 교대 시작 직후 자주 수행하는 업무를 메뉴 탐색보다 먼저 제공하는 기준이다.

| 역할 | Windows 첫 업무 1 | Windows 첫 업무 2 | Windows 첫 업무 3 |
| --- | --- | --- | --- |
| 관리자 | 코멘트 검토 | 보고서 근거 선정 | 동기화·충돌 확인 |
| 반장 | 인수인계 확인 | 문서 찾기·열람 | 작업판 |
| 조장 | 작업 전 문서 확인 | 인수인계 확인 | 채널 알림 |
| 작업자 | 문서 찾기·열람 | 인수인계 확인 | 알림 확인 |

빠른 업무 버튼은 기존 권한 검사와 창을 그대로 사용해야 한다. 관리자 기능이 비활성인 역할에서는 필요한 역할과 `현장 관리자에게 로그인 ID와 필요한 업무를 전달`하는 문의 방법이 화면에 함께 보여야 한다. 권한 역검증은 버튼 활성 상태뿐 아니라 바로 가기, Enter 문서 열기, 메뉴와 보조 창 직접 호출에서도 같은 결과인지 확인한다.
...
알림 복구는 네트워크 단절 중 발생한 메시지를 연결 복구 후 cursor 다음부터 중복 없이 수신하고, 읽음/인수인계 receipt가 서버에 보존되는지 확인한다. Android outbox 대상인 FieldComment/사진은 단절 중 입력, 앱 재시작, 재연결 후 멱등 전송을 확인하며 채널·인수인계가 outbox 대상인 것처럼 오인하지 않는다. 사진 입력은 선택 상태와 미리보기를 보고 올바른 사진인지 확인한 뒤 저장하고, 저장 직후 선택 상태가 비워지는지 함께 본다. 전송 상태 영역에서는 작업자가 대기 건수, 단말 보존 여부, 다음 자동 재시도와 관리자에게 전달할 승인 단말 ID를 도움 없이 찾을 수 있어야 한다. 56dp 주요 버튼은 장갑·한 손·거치 조건에서 실제 터치 성공률과 화면 이동 수를 측정해 판정한다.
...
승인 철회나 중단은 원시 증거를 삭제하거나 운영 템플릿을 초기화하지 않고 승인 이벤트에 추가한다. 철회된 run은 다시 승인하지 않고 새 `run_id`로 시작한다. 중단은 사전 승인한 `stop_criteria` 문구 중 하나와 정확히 같은 기준을 기록해야 한다. 중단 뒤 재개는 `rollback_decision_authority`에 지정한 익명 역할 ID와 접근 통제 저장소의 승인 근거 참조가 있어야 한다.

```powershell
py -3 scripts\manage-pilot-run.py revoke --run-id $RunId --evidence-root $EvidenceRoot --actor-role-id <보안-승인-역할-ID> --reason <철회-사유>
py -3 scripts\manage-pilot-run.py stop --run-id $RunId --evidence-root $EvidenceRoot --actor-role-id <중단-기록-역할-ID> --criterion <승인된-중단-기준-원문> --reason <관찰-사유>
py -3 scripts\manage-pilot-run.py resume --run-id $RunId --evidence-root $EvidenceRoot --actor-role-id <rollback-결정권자-역할-ID> --approval-reference <재개-승인-근거-참조> --reason <재개-사유>
```

`REVOKED` 또는 `STOPPED` 상태에서 `verify`를 실행하면 운영 입력과 완료 판정을 차단한다. `RESUMED` 이벤트가 기록됐더라도 최초 `AUTHORIZED` 계약의 SHA-256과 현재 계약이 다르면 운영 입력과 완료 판정을 차단한다. 계약 범위, 장비, 패키지, RPO/RTO, 담당자 또는 승인자가 바뀌면 기존 원시를 보존하고 새 `run_id`를 발급한다.
...
- `attempt_no`: `BEFORE` 또는 `AFTER` 단계 안에서 중복 없는 1 이상의 시도 번호
- `measurement_status`, `started_at_utc`, `completed_at_utc`: 실제 측정 여부와 시간대가 포함된 시작·완료 시각. `MEASURED` 외 값과 사전 승인보다 이른 시작 시각은 인정하지 않는다.
- `condition_id`, `network`, `gloves`, `one_hand`, `terminal_position`: 같은 시도를 전후에 다시 재현하기 위한 연결·장갑·한 손·거치 위치 조건
- `ui_phase`: `BEFORE` 또는 `AFTER`
- `ui_build`: 실제 실행한 WPF 버전 또는 commit 식별자
- `elapsed_seconds`: 로그인 뒤 업무 완료까지의 실측 시간
- `click_count`: 버튼, 메뉴, 목록 행 등 포인터로 실행한 선택 횟수. 키 입력 수는 포함하지 않는다.
- `screen_transitions`: 새 창, 탭, 메뉴 계층 또는 업무 화면으로 이동한 횟수
- `retry_count`: 같은 업무를 다시 실행하거나 같은 저장을 재전송한 횟수
- `help_request_count`: 관찰자나 관리자에게 사용 방법을 물은 횟수
- `expected_result`, `actual_result`: 기대한 업무 결과와 실제 결과. 비관리자의 검토·보고서 권한 부족 시나리오는 둘 다 `HTTP_403`으로 기록하며 정상 권한 차단을 기능 실패로 세지 않는다.
- `source_preservation_understood`, `next_action_understood`: 권한 부족·동기화 대기·부분 성공·변경 충돌 화면에서 사용자가 원천 보존 여부와 다음 행동을 설명했는지 여부
- `source_loss_count`, `receipt_loss_count`, `duplicate_creation_count`, `critical_blocker`: 원천/처리 결과 유실, 중복 생성, 치명적 차단의 시도별 건수
- `screen_capture_evidence`: 시작 화면, 완료 화면, 실패·보류·충돌 안내가 있으면 해당 화면을 포함한 같은 실행 폴더의 상대경로
- `source_ids`, `source_trace_evidence`: 실제 조회·생성한 문서, FieldComment, 인수인계, 보고서 source 등의 ID와 같은 `run_id`에서 이를 확인한 서버 감사·화면 증거 상대경로

비교할 Windows 시나리오는 다음 세 가지다.

| `scenario_id` | 시작 | 완료 |
| --- | --- | --- |
| `WPF-DOCUMENT-FIELD-COMMENT` | 로그인 완료 | 문서 검색·열람 뒤 FieldComment 저장과 보존/동기화 상태 확인 |
| `WPF-HANDOVER-FOLLOW-UP` | 로그인 완료 | 인수인계 수신 확인 뒤 후속 FieldComment 생성 결과 확인 |
| `WPF-REVIEW-REPORT` | 관리자 로그인 완료 | FieldComment 검토 뒤 보고서에서 근거 선정 확인 |
| `WPF-REVIEW-REPORT-PERMISSION` | 반장·조장·작업자 로그인 완료 | 권한 우회 없이 검토·보고서 권한 부족, 원천 보존, 문의할 다음 행동 확인 |
...
후보 2의 완료 조건은 전체 `full_pilot PASS`가 아니다. 책임자와 승인 계약, 네 역할의 필수 BEFORE 원시 행, 모든 관찰의 개발 항목 전환, 운영·보안·현장 공동 검토를 같은 `run_id`에 확정하는 것이다. `pilot-run.json.ux_before_baseline.status`는 실제 측정과 검토가 끝난 뒤에만 `COMPLETE`로 바꾸고 `review_approvals.operations`, `.security`, `.field_operations`에 각각 `PASS`, 익명 서명자 ID, 시간대 포함 서명 시각과 같은 실행 폴더의 증거를 기록한다.
...
| 공통성 | 공통 제품(`common_product`), 설정·교육(`configuration_or_training`), 현장 배치(`site_layout`) |
| 결정 | 수용(`ACCEPTED`), 불수용(`REJECTED`), 검토(`REVIEW`)와 결정 근거 |
| 분류 근거 | 현장 한 곳의 선호를 공통 제품 변경으로 일반화하지 않았음을 설명하는 `classification_basis` |
| 근거 | `run_id`, 시나리오 ID, 익명 관찰 번호, 로그/화면 시각 |
| 영향 | 영향 역할(`impacted_roles`), 실패 사용자 수, 성공률, 중앙값·최대 소요 시간, 재시도 수 |
| 원천 위험 | 원천·receipt 보존 위험 `NONE`, `POTENTIAL`, `CONFIRMED` 중 하나 |
...
- 관리자·반장·조장·작업자별 필수 시나리오를 2회 이상 반복했고 성공률이 95% 이상이며 중앙값·최대 소요 시간이 실제 관찰값을 근거로 파일럿 승인자의 사전 승인 한도 이내이고 치명적 UX blocker가 0건이다.
...
필수 `scenario_id`는 관리자 `ADMIN-DOCUMENT`, `ADMIN-FIELD-COMMENT-PHOTO`, `ADMIN-WORK-SEQUENCE`, `ADMIN-HANDOVER`, `ADMIN-REVIEW-REPORT`; 반장·조장·작업자는 각각 역할 접두사 `LINE-FOREMAN`, `TEAM-LEAD`, `TEAM-MEMBER`와 `DOCUMENT`, `FIELD-COMMENT-PHOTO`, `WORK-SEQUENCE`, `HANDOVER` 조합을 사용한다. 각 ID에 `required=TRUE`인 시도가 2개 이상 있어야 한다.

| 역할 | 참여자 수 | 필수 시나리오 분모 | 성공 | 성공률 | 중앙값 | 최대 | 재시도 | 도움 요청 | 화면 이동 | 시간 한도 승인 ID/시각 | 치명적 blocker | 판정 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- | ---: | --- |
| 관리자 | 미측정 | 미측정 | 미측정 | 미측정 | 미측정 | 미측정 | 미측정 | 미측정 | 미측정 | 미승인 | 미측정 | 대기 |
| 반장 | 미측정 | 미측정 | 미측정 | 미측정 | 미측정 | 미측정 | 미측정 | 미측정 | 미측정 | 미승인 | 미측정 | 대기 |
| 조장 | 미측정 | 미측정 | 미측정 | 미측정 | 미측정 | 미측정 | 미측정 | 미측정 | 미측정 | 미승인 | 미측정 | 대기 |
| 작업자 | 미측정 | 미측정 | 미측정 | 미측정 | 미측정 | 미측정 | 미측정 | 미측정 | 미측정 | 미승인 | 미측정 | 대기 |

시간 한도는 아직 실제 현장 관찰값과 파일럿 승인 서명이 없으므로 숫자를 임의 확정하지 않는다. 각 역할의 2회 이상 BEFORE 원시 행에서 중앙값·최대값을 계산한 뒤 파일럿 승인자가 `approved_max_median_seconds`, `approved_maximum_seconds`, `time_limit_approval_id`, `time_limit_approved_at`을 승인한다. 이 값은 다음 `run_id`의 통합 승인 전에 확정해야 하며, 도구는 시간 한도 승인 시각이 현재 run 통합 승인 시각보다 늦으면 실패 처리한다. 관찰 후 정한 값을 현재 run의 시험 전 승인으로 소급 기록하지 않는다.
...
복구 전후 증거는 쓰기가 중지되고 SQLite checkpoint 또는 앱 종료가 확인된 상태에서 다음처럼 수집한다. `capture`는 DB `quick_check`·`integrity_check`, foreign key 위반, 모든 업무 테이블의 원천 개수, DB 본파일과 파일 상대경로·크기·SHA-256을 JSON으로 남긴다. schema version 4부터는 Windows `MachineGuid`, macOS `IOPlatformUUID`, Linux `machine-id` 중 현재 OS의 장비 식별값도 읽는다. 원문은 저장하지 않고 SHA-256만 `host_identity`에 기록한다. 수집 전후 DB·WAL·SHM 상태와 파일 집합이 바뀌거나 checkpoint되지 않은 WAL이 남아 있으면 종료 코드 1이다. 전후에는 같은 `--backup-set-id`와 `--restore-approval-id`를 쓰고 서로 다른 익명 `--machine-id`를 기록한다. `compare`는 같은 `run_id`와 대상의 전후 값이 다르거나, `machine_id` 또는 OS 장비 식별 hash가 같거나 비어 있거나, 복구 DB 무결성이 실패하면 종료 코드 1을 반환한다. 같은 실행 경로의 기존 capture·comparison 증거는 덮어쓰지 않는다. 경로와 ID에는 실제 고객명, 서버명, IP를 넣지 않는다.
...
WPF도 같은 명령에서 `--target wpf`, 로컬 DB, `Files` 경로와 서로 다른 원본/복구 PC ID로 전후를 수집한다. 두 comparison이 만들어지면 아래 통합 비교를 실행한다. DB 본파일 크기와 SHA-256은 물리 복사 추적용 참고값으로 남기고, 논리 복구 PASS는 DB 검사·테이블별 row 수와 `storage`/`Files` manifest로 판정한다. 비교 JSON의 `table_count_mismatch_count`와 `file_mismatch_counts.missing/extra/size/sha256`은 모두 0이어야 한다.

```powershell
py -3 scripts\verify-pilot-restore.py compare-set --server D:\FlowNotePilotEvidence\<run_id>\backup-restore\server-comparison.json --wpf D:\FlowNotePilotEvidence\<run_id>\backup-restore\wpf-comparison.json --output D:\FlowNotePilotEvidence\<run_id>\backup-restore\restore-set-comparison.json
```

장애 주입 결과는 `scenario-results/restore-fault-injections.csv`에 `partial_restore`, `old_database_new_files`, `missing_file`, `wrong_server_epoch`를 정확히 한 행씩 기록한다. 각 행은 `target=both`, `automatic_send_blocked`, `polling_blocked`, `reconciliation_required`, `admin_approved_rebind`, `normal_operation_resumed`가 모두 `TRUE`, `result=PASS`여야 한다. `screen_evidence`, `wpf_log_evidence`, `server_audit_evidence`는 같은 실행 폴더 안의 서로 다른 실제 파일을 가리켜야 하며 다른 장애 행과 재사용할 수 없다. 실패 상태와 승인 전 로그는 정상화 결과로 덮어쓰지 않는다.
