...
| 역할 | 필수 시나리오 | 성공 기준 |
| --- | --- | --- |
| 관리자 | 계정·단말 운영, 문서 등록·버전·상태·공개, FieldComment 검토, 보고서 source 선정·저장, 장애 로그 확인 | 승인된 기능만 보이고 모든 변경 주체와 사유가 추적됨 |
...
| 작업자(`team-member`) | 공개 문서 메타데이터 확인, 짧은 FieldComment/사진, 기본 정형 문구, 알림·인수인계 확인 | 비허용 문서·관리 기능 접근 없이 최소 입력으로 완료 |
