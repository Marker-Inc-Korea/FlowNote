...
이 문서는 FlowNote를 개발 PC가 아닌 고객 유사 환경에 설치하고 제한된 현장 파일럿을 시작하려는 조직을 위한 선택 실행 기준이다. 파일럿 수행은 현재 연구개발 완료 조건이 아니다. 다만 실제 운영 적합성을 주장하려면 이 문서의 시나리오와 현장별 승인을 별도로 충족해야 한다.

2026-08-01 현재 저장소에는 리허설·파일럿 증거 생성과 판정을 보조하는 스크립트가 있지만 현장별 서명·인증서·MDM·고객 유사망·별도 PC 복구를 완료했다는 실기 증거는 없다. 따라서 이 문서는 구현 완료 목록이 아니라 현재 코드와 운영 보조 도구에 적용할 후속 실기 절차다.
...
승인이 확정되면 `approvals/pilot-authorization-events.jsonl`에 현재 `run_id`와 계약 SHA-256이 `AUTHORIZED` 이벤트로 추가되고 그때 `scenario-results/android-delivery.csv`, `scenario-results/role-metrics.csv`, `scenario-results/role-ux-comparison.csv`, `scenario-results/restore-fault-injections.csv`, `observations/role-observations.csv`, `observations/development-items.csv`와 설치·복구·Android 운영 템플릿이 생성된다. 이미 있는 파일은 덮어쓰지 않는다. 익명 참여자 ID만 사용해 원시 관찰값을 적고 집계한 분모·성공·중앙값·최대 시간·재시도·도움 요청·화면 이동 수는 `pilot-run.json` schema version 13에 옮긴다. 최종 검증은 요약값을 신뢰하지 않고 역할 원시 행에서 다시 계산해 일치 여부를 확인한다. Android 전달 항목은 정상, Doze, 5분 단절, 재부팅, 서버 주소 변경, access token 만료, refresh 거부, 강제 중지 뒤 kiosk 재실행을 각각 전건 성공과 허용 시간 이내로 기록해야 한다. 정상·Doze의 `allowed_seconds`는 30, 5분 단절의 값은 `30 + page_seconds`로 고정하며 임의 완화할 수 없다. 나머지 시나리오도 사전 승인한 양수 허용 시간과 측정 최대 시간을 반드시 기록한다.
