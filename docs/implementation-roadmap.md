...
이 문서는 2026-07-15 현재 코드에서 완료된 범위와, 아직 구현되지 않은 예외 작업을 분리한다. “완료된 기반”은 코드가 존재하는 항목이며 운영 실기 통과를 뜻하지 않는다.
...
1. 현재 106건을 수집하는 FastAPI 테스트 코드에 맞춰 `verify-preserved-tests.ps1`의 104건 고정 기대값을 갱신한 뒤, 전체 pytest, WPF 빌드·스모크와 Git 산출물 사후 점검을 포함한 표준 검증을 실행하고 결과를 남긴다.
...
- 외부 AI 호출 기반 검색/작업 조언. 착수 기준은 [MVP 범위 문서의 후속 계층 착수 기준](./mvp-scope.md#후속-계층-착수-기준)을 따른다.
  ...
  - 질의 조회 범위, 만료 보존 자동 스케줄러와 현장 운영 절차를 확정한다. 현재 만료 처리는 `system-admin`이 API/WPF에서 수동 실행할 수 있다.
- 운영 감사 로그와 보안 정책 고도화
