...
1. 현재 코드와 표준 스크립트 guard는 FastAPI 166건·WPF Core 91건·Android 28건으로 일치한다. FastAPI 수집/JUnit·도구 부족 실패 안내와 의도적 불일치 단위 검증도 보강했다. 다음에는 Windows x64에서 FastAPI 수집 총 166건·고유 166건·중복 0건과 JUnit 166/166, WPF Core 수집 목록과 TRX `total/passed=91/91`, Android JUnit 28/28을 대조한다. 이어 누적 공통 DB 스모크 전후 무결성, Android debug build와 Git 사후 점검을 생략 없이 실행해 `partial_run=false`, `PASSED` 실행 ID를 같은 clean 소스 커밋에서 2회 연속 남겨야 한다. 현재 최신 유효 기준선과 재현 실행은 모두 `대기`다.
