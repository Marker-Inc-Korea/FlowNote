...
1. 현재 코드는 FastAPI 150건·WPF Core 45건을 수집하지만 `scripts/verify-preserved-tests.ps1` guard는 149건·43건이다. 먼저 guard를 현재 수치로 보정한 뒤 Windows x64 표준 환경에서 누적 공통 DB 스모크 전후 무결성, Android 단위 테스트·debug build와 Git 사후 점검을 생략 없이 실행해 `partial_run=false`, 단일 `PASSED` 실행 ID를 남긴다.
