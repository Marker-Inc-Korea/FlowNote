...
1. 현재 코드와 `scripts/verify-preserved-tests.ps1` guard는 FastAPI 154건·WPF Core 67건·Android 16건으로 일치한다. Windows x64 표준 환경에서 누적 공통 DB 스모크 전후 무결성, Android 단위 테스트·debug build와 Git 사후 점검을 생략 없이 실행해 `partial_run=false`, `PASSED` 실행 ID를 같은 clean 소스 커밋에서 2회 연속 남긴다.
