...
이 문서는 2026-07-28 현재 코드에서 완료된 범위와, 아직 구현되지 않은 예외 작업을 분리한다. “완료된 기반”은 코드가 존재하는 항목이며 운영 실기 통과를 뜻하지 않는다.
...
1. 현재 코드는 FastAPI 154건·WPF Core 76건이 통과했지만 표준 스크립트 guard는 FastAPI 154건·WPF Core 74건·Android 20건이다. WPF Core guard를 현재 코드에 맞추고 Windows 수집 목록과 TRX `total/passed=76/76`을 대조해야 한다. Windows x64 표준 환경에서 누적 공통 DB 스모크 전후 무결성, Android 단위 테스트·debug build와 Git 사후 점검을 생략 없이 실행해 `partial_run=false`, `PASSED` 실행 ID를 같은 clean 소스 커밋에서 2회 연속 남겨야 한다. 현재 최신 유효 기준선과 재현 실행은 모두 `대기`다.
...
11. 고객 승인을 받은 익명 현장 원천으로 독립 2인 승인 ground-truth 48건을 구성하고 동일 snapshot 2회 전체 회귀와 24칸 독립 표본 검토·필요 시 제3 합의, provider 심사를 실제 현장 준비도 기준으로 통과시킨다. 합성/시험 스모크와 `PILOT`은 별도 회귀·파일럿 기록으로만 유지한다.
