...
1. 현재 코드와 표준 스크립트 guard는 FastAPI 154건·WPF Core 76건·Android 20건으로 맞는다. Windows 수집 목록과 TRX `total/passed=76/76`을 대조하고 Windows x64 표준 환경에서 누적 공통 DB 스모크 전후 무결성, Android 단위 테스트·debug build와 Git 사후 점검을 생략 없이 실행해 `partial_run=false`, `PASSED` 실행 ID를 같은 clean 소스 커밋에서 2회 연속 남겨야 한다. 현재 최신 유효 기준선과 재현 실행은 모두 `대기`다.
