...
- AI 검색 전 단계의 근거 후보 재생성과 품질 점검 API
...
외부 AI 호출 기반 검색/작업 조언, MES/ERP 연동, 관리자 강제 세션 폐기 UI, 서버 파일 감시 API는 현재 구현이 아니라 후속 범위이다. 현재 구현된 AI 관련 범위는 `ai_search_candidates` 근거 후보 read model과 품질 점검 API까지다.
