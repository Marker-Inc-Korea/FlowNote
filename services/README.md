...
이 문서는 2026-07-14 현재 `services/api` 코드 기준이다. 구현되지 않은 서비스는 마지막 후속 범위에서만 예외로 다룬다.
...
provider 직전 게이트는 `ai_sensitive_data_policies`의 활성 고객·현장 정책을 읽어 금칙어와 고객 식별자를 원천 단위로 차단한다. 정책 등록·변경용 운영 API/UI는 현재 서비스 범위에 없다.
