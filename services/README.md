...
이 문서는 2026-07-21 현재 `services/api` 코드 기준이다. 구현되지 않은 서비스는 마지막 후속 범위에서만 예외로 다룬다.
...
- 문서 태그 등록, 목록, 교체
- FieldComment 원천 불변 등록, 목록, 상세, 단계형 관리자 검토, 담당자·기한, 최대 200건 일괄 변경, 원천 hash 감사와 품질 작업함/지표
