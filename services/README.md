...
이 문서는 2026-08-01 현재 `services/api` 코드 기준이다. 구현되지 않은 서비스는 마지막 후속 범위에서만 예외로 다룬다.
...
- 문서 태그 등록, 목록, 교체
- FieldComment 원천 불변 등록, 목록, 상세, 단계형 관리자 검토, 담당자·기한, 최대 200건 일괄 변경, 원천 hash 감사와 품질 작업함/지표. 검토 대시보드는 분석 권한 역할에 미검토·상충·안전/품질 위험·보고서 미연결·담당자 없음의 현재 집계와 담당 역할·다음 조치를 반환한다. 개별 검토는 `review_revision` 조건부 갱신과 mutation receipt를 사용한다.
