...
이 문서는 2026-08-09 현재 `services/api` 코드 기준이다. 구현되지 않은 서비스는 마지막 후속 범위에서만 예외로 다룬다.

승인된 Windows 서버의 설치·일상 운영 순서는 [서버 설치·운영 매뉴얼](../docs/manuals/server-operations.md)을 따른다.
...
- 문서 상태 변경, 버전 상태 변경, 명시적 공개 버전 지정, 공개 문서 조회. 공개·상태·태그·삭제는 revision·mutation receipt를 사용하고 문서 태그는 비경합 추가·제거 delta만 병합
- 문서 검토 요청, 지정 검토자의 승인·반려, 승인된 exact version 공개·취소와 append-only 승인 이력. 요청·결정·공개는 version·문서 revision·file hash와 역할 분리 설정을 다시 확인
- 문서 태그 등록과 목록
- FieldComment 원천 불변 등록, 목록, 상세, 단계형 관리자 검토, 담당자·기한, 최대 200건 일괄 변경, 원천 hash 감사와 품질 작업함/지표·통합 역추적. 검토 대시보드는 분석 권한 역할에 미검토·상충·안전/품질 위험·보고서 미연결·담당자 없음의 현재 집계와 담당 역할·다음 조치를 반환한다. 개별 검토는 `review_revision` 조건부 갱신과 mutation receipt를 사용한다.
...
provider 직전 게이트는 `ai_sensitive_data_policies`의 활성 고객·현장 정책을 읽어 금칙어와 고객 식별자를 원천 단위로 차단한다. 민감정보 정책 운영 API는 원문이 불변인 버전의 작성·분리 검토·승인·활성·대체·철회·폐기와 정제 감사를 제공한다. provider 호출 직전과 응답 직후에는 활성 정책의 ID·content hash·revision snapshot을 다시 확인한다. 별도로 구현된 `ai_operational_policies` API/UI는 kill switch, 호출 한도, 보존 기간과 감사 내보내기 허용 여부를 관리한다.
