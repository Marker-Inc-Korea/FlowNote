...
이 문서는 2026-08-03 현재 `services/api` 코드 기준이다. 구현되지 않은 서비스는 마지막 후속 범위에서만 예외로 다룬다.
...
- 문서 권위 변경, FieldComment 검토, 보고서 상태 전이와 작업순서 변경의 공통 mutation receipt·감사 event envelope 조회 및 현재 업무 상태와 결합한 권한 기반 통합 변경 이력
