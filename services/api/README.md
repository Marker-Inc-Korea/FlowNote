...
| GET | `/api/v1/ai-operations/sensitive-data-policies` | List redacted sensitive-policy versions |
| POST | `/api/v1/ai-operations/sensitive-data-policies` | Create an immutable sensitive-policy draft |
...
`GET /api/v1/tags` requires any active authenticated account because equipment, line, process, and error tags can expose a site's operating structure. Creating tags requires a document-write role. All document, FieldComment, access log, work sequence, report, and AI search candidate endpoints use the authentication and role policies described in [docs/api.md](../../docs/api.md).
...
- `FLOWNOTE_INITIAL_ADMIN_PASSWORD`: required only when a fresh server DB creates its first `admin`; leave no value in committed files
- `FLOWNOTE_ACCESS_TOKEN_SECRET`: non-`local`/`test` environments reject the public placeholder and values shorter than 32 characters
...
The current `scripts/verify-preserved-tests.ps1` guards are 212 FastAPI cases, 120 WPF Core cases, and 39 Android unit tests. The repository does not publish prior test databases, logs, uploads, run IDs, or raw results. Run the commands in [the testing guide](../../docs/verification.md) from a clean clone and judge the checked-out commit from that execution. The same guide explains local artifact cleanup and the separate approved-HTTPS smoke procedure.
...
Test SQLite DBs, logs, upload files, and generated samples are Git-ignored. Use `python scripts/reset_local_test_data.py` from the repository root to preview cleanup and add `--apply` to reset them.
