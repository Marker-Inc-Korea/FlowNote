...
As of 2026-07-27, FastAPI passes 154 unique node IDs, WPF Core passes 67 tests, and Android passes 16 unit tests. The `scripts/verify-preserved-tests.ps1` guard expects the same 154/67/16 counts. The latest WPF additions cover the four restore-fault guidance paths, explicit manifest fault blocking, and approval-gated rejoin followed by normal-state recovery for each fault. Future count changes must be justified by test changes and matched against FastAPI collection/unique node IDs, pytest JUnit, WPF TRX, and Android JUnit. Ruff passed for `app` and `tests`, and the WPF app built without warnings or errors on the macOS ARM64 host. This is not an integrated baseline because the shared-DB smoke and all checks were not run twice under one clean source commit on Windows x64.
