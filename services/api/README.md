...
As of 2026-07-27, FastAPI passes 154 unique node IDs, WPF Core passes 71 tests, and Android passes 16 unit tests. The `scripts/verify-preserved-tests.ps1` guard still expects 67 WPF Core tests, so it must be reconciled with the current collection and TRX count before the next standard Windows run. Future count changes must be justified by test changes and matched against FastAPI collection/unique node IDs, pytest JUnit, WPF TRX, and Android JUnit. Ruff passed for `app` and `tests`, and the WPF app built without warnings or errors on the macOS ARM64 host. This is not an integrated baseline because the shared-DB smoke and all checks were not run twice under one clean source commit on Windows x64.
