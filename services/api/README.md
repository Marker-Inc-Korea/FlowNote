...
As of 2026-07-30, the current code and `scripts/verify-preserved-tests.ps1` guards are aligned at 154 FastAPI tests, 76 WPF Core tests, and 20 Android tests. The macOS collection contained 76 unique WPF Core tests and its TRX reported `total/passed=76/76`. Future count changes must be justified by test changes and matched against FastAPI collection/unique node IDs, pytest JUnit, WPF collection/unique IDs and TRX, and Android JUnit. This is not an integrated baseline because the shared-DB smoke and all checks have not run twice under one clean source commit on Windows x64.
