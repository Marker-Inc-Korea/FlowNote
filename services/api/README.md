...
As of 2026-07-28, FastAPI passes 154 unique node IDs, WPF Core passes 74 tests on macOS, and Android passes 20 unit tests. The `scripts/verify-preserved-tests.ps1` guard now expects the same 154/74/20 counts. The WPF Core collection and TRX `total/passed=74/74` result still need to be matched on Windows x64. Future count changes must be justified by test changes and matched against FastAPI collection/unique node IDs, pytest JUnit, WPF TRX, and Android JUnit. Ruff passed for `app` and `tests`, and the WPF app built without warnings or errors on the macOS ARM64 host. This is not an integrated baseline because the shared-DB smoke and all checks were not run twice under one clean source commit on Windows x64.
