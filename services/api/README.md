...
As of 2026-07-30, the current code contains 160 FastAPI tests, 84 WPF Core tests, and 24 Android tests. The actual `scripts/verify-preserved-tests.ps1` guard constants are 155, 84, and 24 respectively, while its FastAPI step guidance says 160. The FastAPI guard and guidance must be aligned with the current collection before the script can produce a valid run. The current macOS component runs passed all 160 FastAPI tests and all 84 WPF Core tests. This is not an integrated baseline because the Windows x64 collection/TRX comparison, shared-DB smoke, Android build, and all checks have not run twice under one clean source commit.
